...
# ▼▼▼ SUBSTITUA O BLOCO 'if not settings.configured:' INTEIRO POR ESTE CÓDIGO ▼▼▼
if not settings.configured:
    settings.configure(
        ...
        INSTALLED_APPS=[
            ...
            'rest_framework',
            'channels',
            'main',  # <<< CORREÇÃO: DE '__name__' PARA 'main', IGUAL AO app_label
        ],
...
class Receita(TenantModel):
    ...
    processado_por_id = models.IntegerField(null=True, blank=True, help_text="ID do usuário que marcou como recebida")
    processado_por_nome = models.CharField(max_length=200, blank=True, null=True, help_text="Nome do usuário que marcou como recebida")
...
class Despesa(TenantModel):
    ...
    processado_por_id = models.IntegerField(null=True, blank=True, help_text="ID do usuário que marcou como paga")
    processado_por_nome = models.CharField(max_length=200, blank=True, null=True, help_text="Nome do usuário que marcou como paga")
...
def gerar_documento_medico_sync(consulta_id, tipo_documento, medico_nome, medico_crm):
    ...
    try:
        ...
        if consulta.documentos_gerados:
            for doc in consulta.documentos_gerados:
                if doc.get('tipo') == tipo_documento:
                    print(f"✅ Documento '{tipo_documento}' já existe em cache, retornando...")
                    return {
                        'sucesso': True,
                        'tipo': tipo_documento,
        ...
        try:
            ...
            print(f"⚠️ Aviso ao configurar Gemini: {str(e)}")
        
        try:
            model = genai.GenerativeModel('gemini-2.5-pro')
        except Exception as e:
            raise Exception(f"Erro ao criar modelo Gemini: {str(e)}")
        ...
        largura_linha = 80
        assinatura_linha = "_" * (len(medico_nome) + 8)

        assinatura_bloco = f"""
{assinatura_linha.center(largura_linha).rstrip()}
{medico_nome.center(largura_linha).rstrip()}
{medico_funcao.center(largura_linha).rstrip()}
{f'CRM: {medico_crm}'.center(largura_linha).rstrip()}
"""
        # ▲▲▲ FIM DA ALTERAÇÃO ▲▲▲

        prompts = {
            'atestado': f"""
Você é um assistente médico que gera APENAS documentos baseados em FATOS EXPLICITAMENTE MENCIONADOS.

⚠️ REGRAS ABSOLUTAS:
- NUNCA invente, infira ou complete informações que não foram ditas
- NÃO use formatação Markdown (asteriscos, hashtags, etc)
- Gere texto LIMPO e profissional

DADOS DISPONÍVEIS:
- Paciente: {paciente.nome_completo}
- CPF: {paciente.cpf}
- Data Nascimento: {paciente.data_nascimento.strftime('%d/%m/%Y')}
- Data da Consulta: {data_extenso}
- Queixa: {consulta.queixa_principal or '[Não informada na transcrição]'}
- Diagnóstico: {consulta.diagnostico or consulta.hipotese_diagnostica or '[Não informado na transcrição]'}

INSTRUÇÕES:
1. Gere um ATESTADO MÉDICO profissional
2. Use APENAS as informações acima
3. Se dias de afastamento NÃO foram mencionados, use texto genérico: "para os devidos fins"
4. NÃO invente CID, dias de repouso ou outras informações não mencionadas
5. NÃO use asteriscos, hashtags ou qualquer formatação Markdown
6. Texto simples e limpo

FORMATO:
ATESTADO MÉDICO

Atesto para os devidos fins que o(a) paciente [NOME], portador(a) do CPF [CPF], nascido(a) em [DATA], esteve sob meus cuidados médicos em [DATA DA CONSULTA].

[Mencione apenas o que foi explicitamente dito sobre diagnóstico e conduta]
...
""",
            ...
            'anamnese': f"""
Você é um assistente médico que transcreve anamneses baseadas APENAS em informações EXPLICITAMENTE mencionadas.

⚠️ REGRAS ABSOLUTAS: 
- NÃO preencha campos que não foram mencionados
- NÃO infira informações
- NÃO complete com dados "típicos" ou "esperados"
- Use "[Não informado]" ou deixe vazio se não foi mencionado
- NÃO use formatação Markdown (sem asteriscos, sem hashtags)
...
INSTRUÇÕES:
1. Organize a transcrição em formato de anamnese
2. Use APENAS informações que estão na transcrição acima
3. Para cada seção, coloque apenas o que foi mencionado
4. Se uma seção não tem informação, escreva "[Não avaliado na consulta]"
5. Texto simples, SEM formatação Markdown

ESTRUTURA (use apenas o que tem informação):
1. Identificação: [Use dados do paciente acima]
2. Queixa Principal: [Apenas se mencionado]
3. História da Doença Atual: [Apenas o que foi dito]
4. Revisão de Sistemas: [Apenas se mencionado]
5. Antecedentes: [Apenas se mencionado]
6. Exame Físico: [Apenas o que foi realizado]
7. Hipótese Diagnóstica: [Apenas se mencionado]

{assinatura_bloco}
""",
            
            'evolucao': f"""
Você é um assistente médico que documenta evoluções médicas baseadas APENAS em FATOS EXPLICITAMENTE MENCIONADOS.

⚠️ REGRAS ABSOLUTAS:
- NÃO invente, NÃO infira, NÃO complete informações
- NÃO use formatação Markdown (sem asteriscos, sem hashtags, sem sublinhados)
- Texto limpo e profissional
...
- Diagnóstico: {consulta.diagnostico or '[Não definido]'}
- Conduta: {consulta.conduta or '[Não definida]'}

INSTRUÇÕES:
1. Documente no formato SOAP
2. Use APENAS informações explicitamente mencionadas
3. NÃO adicione sinais vitais, estado geral ou outros dados não mencionados
4. Se algo não foi dito, escreva "[Não avaliado]"
5. Texto simples, SEM formatação Markdown
...
S (Subjetivo): 
[Apenas o que o paciente relatou na transcrição]

O (Objetivo): 
[Apenas achados explicitamente mencionados no exame físico]
[Se PA foi dita, inclua. Se não, NÃO invente]

A (Avaliação): 
[Apenas diagnóstico/hipótese se mencionado]

P (Plano): 
[Apenas conduta explicitamente mencionada]

{assinatura_bloco}
""",
            
            'prescricao': f"""
Você é um assistente médico que gera prescrições baseadas APENAS em medicamentos EXPLICITAMENTE MENCIONADOS.

⚠️ REGRAS ABSOLUTAS:
- Se medicamentos foram mencionados na transcrição, GERE A PRESCRIÇÃO completa
- Se dosagem foi mencionada, use-a. Se não, use dosagem padrão do medicamento
...
TRANSCRIÇÃO DO MÉDICO:
{consulta.transcricao_medico or '[Sem transcrição do médico]'}

CONDUTA REGISTRADA:
{consulta.conduta or '[Nenhuma conduta registrada]'}
...
INSTRUÇÕES:
1. Analise TODA a transcrição procurando por medicamentos mencionados
2. Se encontrou medicamentos, GERE a prescrição completa:
   - Nome do medicamento
   - Dosagem (se mencionada) ou dosagem padrão
   - Via de administração
   - Posologia (horários de uso)
   - Duração do tratamento (se mencionada)
3. Se NÃO encontrou NENHUM medicamento mencionado, escreva apenas: "Sem medicamentos prescritos nesta consulta"
4. Texto simples, SEM formatação Markdown
...
[Se encontrou medicamentos, liste-os aqui com todas as informações]
[Se não encontrou, escreva: "Sem medicamentos prescritos nesta consulta"]

Orientações:
[Apenas se orientações foram explicitamente mencionadas]

{assinatura_bloco}
""",
            
            'relatorio': f"""
Você é um assistente médico que gera relatórios baseados APENAS em informações EXPLICITAMENTE DOCUMENTADAS.

⚠️ REGRAS ABSOLUTAS:
- Este é um relatório FACTUAL. NÃO invente informações
- NÃO use formatação Markdown (sem asteriscos, sem hashtags, sem sublinhados)
- Texto limpo e profissional
...
- Queixa: {consulta.queixa_principal or '[Não registrada]'}
- História: {consulta.historia_doenca_atual or '[Não registrada]'}
- Exame Físico: {consulta.exame_fisico or '[Não realizado]'}
- Hipótese Diagnóstica: {consulta.hipotese_diagnostica or '[Não definida]'}
- Conduta: {consulta.conduta or '[Não definida]'}
...
INSTRUÇÕES:
1. Gere um relatório médico profissional
2. Use APENAS informações acima
3. NÃO adicione informações não mencionadas
4. Se algo não foi avaliado/registrado, deixe claro no relatório
5. Texto simples, SEM formatação Markdown
...
HISTÓRIA CLÍNICA:
[Apenas informações da transcrição]

EXAME FÍSICO:
[Apenas achados documentados]

CONCLUSÃO:
[Apenas diagnóstico/hipótese se mencionado]

CONDUTA:
[Apenas se mencionada]
...
"""
        ...
        try:
            ...
            error_msg = str(e)
            print(f"❌ Erro ao gerar com Gemini: {error_msg}")
            
            if "quota" in error_msg.lower() or "rate" in error_msg.lower() or "429" in error_msg:
                print("⏳ Aguardando 60 segundos devido a rate limit excedido...")
                time.sleep(60)
                try:
                    response = model.generate_content(prompt, request_options=request_options)
                    documento = response.text
                except Exception as retry_error:
                    raise Exception(f"Falha após retry: {str(retry_error)}")
            else:
                raise
        
        if '```' in documento:
            documento = re.sub(r'```[\w]*\n', '', documento)
            documento = documento.replace('```', '')
        
        documento = re.sub(r'\*\*([^\*]+)\*\*', r'\1', documento)
        documento = re.sub(r'\*([^\*]+)\*', r'\1', documento)
        documento = re.sub(r'__([^_]+)__', r'\1', documento)
        documento = re.sub(r'_([^_]+)_', r'\1', documento)
        documento = re.sub(r'^#{1,6}\s+', '', documento, flags=re.MULTILINE)
        documento = re.sub(r'\n{3,}', '\n\n', documento)
...
def login_view(request):
    ...
    # <<< INÍCIO DA ALTERAÇÃO >>>
    # Obter o nome da clínica para incluir na resposta do login.
    # Se o usuário for super_admin, o nome da clínica será nulo.
    ...
    return Response({
        ...
        'user_info': {
            ...
            'clinica_nome': clinica_nome,  # <<< CAMPO ADICIONADO >>>
...
class AgendamentoSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Agendamento
        fields = [
            'id', 'clinica_id', 'paciente', 'paciente_id', 'paciente_nome', 'paciente_cpf',  # ← ADICIONE paciente_id aqui
...
class AgendamentoListSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'status_display',
            'convenio',
            # NOVOS CAMPOS ADICIONADOS À RESPOSTA DA API
            'paciente_telefone',
            'paciente_sexo',
            'paciente_idade'
        ]
...
class ExameSerializer(serializers.ModelSerializer):
    ...
    def __init__(self, *args, **kwargs):
        ...
        """
        # Executa o construtor padrão da classe pai primeiro
        super(ExameSerializer, self).__init__(*args, **kwargs)
        
        # Obtém o objeto 'request' do contexto do serializador
        request = self.context.get('request')
        
        # Verifica se o request e o usuário existem
        if request and hasattr(request, 'user') and request.user:
            # Obtém a função do payload do token JWT do usuário
...
# ▼▼▼ SUBSTITUA A CLASSE 'ReceitaListSerializer' PELA VERSÃO ABAIXO ▼▼▼
...
class AgendamentoViewSet(viewsets.ModelViewSet):
    ...
    # ▼▼▼ CORREÇÃO APLICADA AQUI ▼▼▼
    ordering = ['data', 'hora'] # Removemos o '-' para ordenar de forma crescente
    # ▲▲▲ FIM DA CORREÇÃO ▲▲▲
    ...
    def get_queryset(self):
        ...
        queryset = Agendamento.objects.select_related('paciente').filter(clinica_id=clinica_id)
    ...
    def destroy(self, request, *args, **kwargs):
        ...
        # Deleta consultas e receitas associadas ANTES de deletar o agendamento
        Consulta.objects.filter(agendamento=instance).delete()
        Receita.objects.filter(agendamento=instance).delete()
        
        # Agora deleta o próprio agendamento
        self.perform_destroy(instance)
        
        # Retorna uma resposta de sucesso sem conteúdo, como é o padrão para DELETE
...
# ▼▼▼ SUBSTITUA A CLASSE 'ConsultaViewSet' INTEIRA PELA VERSÃO ABAIXO ▼▼▼
...
class ReceitaViewSet(viewsets.ModelViewSet):
    ...
    def partial_update(self, request, *args, **kwargs):
        """
        Sobrescreve a atualização parcial para setar a data de recebimento
        e o usuário responsável automaticamente quando o status for alterado para 'recebida'.
        """
        response = super().partial_update(request, *args, **kwargs)
        
        if response.status_code == 200 and 'status' in request.data:
            novo_status = request.data.get('status')
            if novo_status == 'recebida':
                instance = self.get_object()
                
                # Preenche a data de recebimento se não houver
                if not instance.data_recebimento:
                    instance.data_recebimento = date.today()
                
                # Captura o usuário que realizou a ação
                user_payload = request.user.payload
                instance.processado_por_id = user_payload.get('sub')
                instance.processado_por_nome = user_payload.get('nome')
                
                instance.save(update_fields=['data_recebimento', 'processado_por_id', 'processado_por_nome'])
    ...
    def receber_de_agendamento(self, request):
        ...
        receita = Receita.objects.create(clinica_id=clinica_id, descricao=f"{agendamento.servico} - {agendamento.paciente_nome}", valor=agendamento.valor, categoria=categoria, data_vencimento=agendamento.data, data_recebimento=date.today(), status='recebida', agendamento=agendamento, paciente=agendamento.paciente, forma_pagamento=forma_pagamento, observacoes=f"Recebimento de {agendamento.tipo} - {agendamento.data}")
        serializer = ReceitaSerializer(receita, context={'request': request})
...
class DespesaViewSet(viewsets.ModelViewSet):
    ...
    def partial_update(self, request, *args, **kwargs):
        """
        Sobrescreve a atualização parcial para setar a data de pagamento
        e o usuário responsável automaticamente quando o status for alterado para 'paga'.
        """
        response = super().partial_update(request, *args, **kwargs)
        ...
        if response.status_code == 200 and 'status' in request.data:
            ...
            if novo_status == 'paga':
                ...
                instance.save(update_fields=['data_pagamento', 'processado_por_id', 'processado_por_nome'])
...
# ▼▼▼ SUBSTITUA A FUNÇÃO 'dashboard_financeiro' INTEIRA PELA VERSÃO ABAIXO ▼▼▼
@api_view(['GET'])
@authentication_classes([JWTAuthentication]) 
@permission_classes([IsAuthenticated, IsSecretariaOrAbove])
def dashboard_financeiro(request):
    ...
    # Filtros para o que VENCE no período (para A Receber/A Pagar/Vencidas)
    filtros_vencimento = {}
    if data_inicial and data_final:
        filtros_vencimento['data_vencimento__range'] = [data_inicial, data_final]
    ...
    # Filtros para o que foi PAGO/RECEBIDO no período (para cards principais e caixa)
    filtros_efetivados_receita = {'status': 'recebida'}
    filtros_efetivados_despesa = {'status': 'paga'}
    if data_inicial and data_final:
        filtros_efetivados_receita['data_recebimento__range'] = [data_inicial, data_final]
        filtros_efetivados_despesa['data_pagamento__range'] = [data_inicial, data_final]

    # Cálculos para os cards do dashboard (agora usando os filtros corretos)
    ...
    # Cálculos de pendências continuam baseados no vencimento
    ...
    # Previsão considera tudo que vence no período, realizado ou não
    ...
    # Lógica de Fechamento de Caixa (agora usando os filtros corretos)
    ...
    # Itera sobre todos os métodos de pagamento para garantir que todos apareçam se tiverem transação
...
# ▼▼▼ SUBSTITUA A FUNÇÃO 'inicializar_banco' INTEIRA PELA VERSÃO ABAIXO ▼▼▼
def inicializar_banco():
    ...
    try:
        # Força a geração de migrações e a aplicação delas.
    ...
    try:
        ...
        print(f"❌ Erro ao verificar ou criar super admin: {e}")
    ...
    for exame in exames_criados:
        ...
        if ts:
            eventos.append({
                ...
                'detalhes': f"Tipo: {exame.get_tipo_exame_display()}"
            })
    ...
    for exame in exames_revisados:
        ...
        if ts:
            eventos.append({
                ...
                'detalhes': f"Revisão do Laudo de {exame.get_tipo_exame_display()}"
            })
...
def auditoria_financeira_view(request):
    """
    Retorna um log de transações financeiras (receitas recebidas e despesas pagas)
    para auditoria, com filtro por mês e ano.
    ...
    try:
        ...
        return Response({"erro": "Parâmetros de mês e ano inválidos."}, status=400)

    eventos = []

    # Buscar receitas recebidas no período
    receitas_pagas = Receita.objects.filter(
        clinica_id=clinica_id,
        status='recebida',
    ...
    for r in receitas_pagas:
        eventos.append({
            'timestamp': r.data_recebimento,
            'tipo': 'Receita',
            'descricao': r.descricao,
            'valor': float(r.valor),
            'usuario_nome': r.processado_por_nome or 'Não identificado'
        })
    ...
    for d in despesas_pagas:
        eventos.append({
            'timestamp': d.data_pagamento,
            'tipo': 'Despesa',
            'descricao': d.descricao,
            'valor': float(d.valor),
            'usuario_nome': d.processado_por_nome or 'Não identificado'
        })

    # Ordenar todos os eventos pela data
    eventos_ordenados = sorted(eventos, key=lambda x: x['timestamp'], reverse=True)

    # Formatar para a resposta final
    eventos_finais = [
        {
            'data': evt['timestamp'].strftime('%d/%m/%Y'),
            'tipo': evt['tipo'],
            'descricao': evt['descricao'],
            'valor_formatado': f"R$ {format_currency_brl(evt['valor'])}",
            'usuario_nome': evt['usuario_nome']
        }
