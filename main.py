...
if __name__ == "__main__":
    # Isso impede que o Django carregue o arquivo duas vezes
    # e cause o erro de conflito de Models
    import sys
...
def registrar_consumo_ia(clinica_id, tipo_consumo, **kwargs):
    """Registra um novo consumo de IA no banco de dados."""
    try:
        ...
        # É importante logar o erro, mas não quebrar a aplicação principal
        print(f"ERRO CRÍTICO: Não foi possível registrar o consumo de IA para a clínica {clinica_id}. Erro: {e}")

def verificar_limite_ia(tipo_consumo):
    """
    Decorator para verificar se a clínica pode usar uma funcionalidade de IA.
    Ele verifica o limite total mensal/período.
    """
    def decorator(view_func):
        ...
        def _wrapped_view(*args, **kwargs):
            ...
            if not clinica_id:
                return Response({'erro': 'Acesso negado. Usuário não vinculado a uma clínica.'}, status=status.HTTP_403_FORBIDDEN)

            try:
                assinatura = AssinaturaClinica.objects.select_related('plano').get(clinica_id=clinica_id, status='ativa')
            except AssinaturaClinica.DoesNotExist:
                return Response({'erro': 'Sua clínica não possui um plano de assinatura ativo para usar este recurso.'}, status=status.HTTP_403_FORBIDDEN)

            plano = assinatura.plano
            hoje = timezone.now().date()
            
            if not (assinatura.data_inicio <= hoje <= assinatura.data_fim):
                assinatura.status = 'expirada'
                assinatura.save()
                return Response({'erro': 'Seu plano de assinatura expirou. Contate o suporte para renovar.'}, status=status.HTTP_403_FORBIDDEN)

            limite_map = {
                'transcricao_consulta': plano.limite_transcricao_consulta_mes,
                'transcricao_exame_geral': plano.limite_transcricao_exame_mes,
                'laudo_exame_ia': plano.limite_laudo_exame_mes,
                'documento_medico_ia': plano.limite_documento_ia_mes,
            }
            limite_total_periodo = limite_map.get(tipo_consumo, 0)

            consumo_total = ConsumoIA.objects.filter(
                clinica_id=clinica_id,
                tipo_consumo=tipo_consumo,
                data_consumo__date__range=[assinatura.data_inicio, assinatura.data_fim]
            ).count()

            if consumo_total >= limite_total_periodo:
                # ▼▼▼ MENSAGEM DE ERRO ATUALIZADA AQUI ▼▼▼
                error_msg = (
                    f"Limite mensal de '{ConsumoIA.TIPO_CONSUMO_CHOICES_DICT.get(tipo_consumo, tipo_consumo)}' atingido ({consumo_total}/{limite_total_periodo}). "
                    "Para continuar utilizando este recurso, considere fazer um upgrade de plano."
                )
                return Response({'erro': error_msg}, status=status.HTTP_403_FORBIDDEN)
                # ▲▲▲ FIM DA ATUALIZAÇÃO ▲▲▲

            response = view_func(*args, **kwargs)
            
            if 200 <= response.status_code < 300:
                detalhes = { 'usuario_id': user.get('sub') }
                if 'pk' in kwargs:
                    if 'consultas' in request.path: detalhes['consulta_id'] = kwargs['pk']
                    elif 'exames' in request.path: detalhes['exame_id'] = kwargs['pk']
                
                registrar_consumo_ia(clinica_id, tipo_consumo, **detalhes)
...
class IsAdminOrMedico(BasePermission):
    ...
    def has_permission(self, request, view):
        ...
        funcoes = request.user.get('funcoes', [])
        return any(f in funcoes for f in ['super_admin', 'admin', 'medico'])
        # --- FIM DA ALTERAÇÃO ---
...
class IsSecretariaOrAbove(BasePermission):
    ...
    def has_permission(self, request, view):
        ...
        # --- ALTERAÇÃO APLICADA AQUI ---
        funcoes = request.user.get('funcoes', [])
        return any(f in funcoes for f in ['super_admin', 'admin', 'medico', 'secretaria'])
        # --- FIM DA ALTERAÇÃO ---
...
class IsAdminOnly(BasePermission):
    ...
    def has_permission(self, request, view):
        ...
        # --- ALTERAÇÃO APLICADA AQUI ---
        funcoes = request.user.get('funcoes', [])
        return any(f in funcoes for f in ['super_admin', 'admin'])
        # --- FIM DA ALTERAÇÃO ---
...
class Agendamento(TenantModel):
    ...
    # --- ALTERAÇÃO APLICADA AQUI ---
    # A restrição 'limit_choices_to' foi removida
    ...
    def clean(self):
        ...
        if self.medico_responsavel and 'medico' not in self.medico_responsavel.funcoes:
            errors['medico_responsavel'] = 'O usuário selecionado não tem a função de médico.'
...
class Clinica(models.Model):
    ...
    # Configuração de Alertas de Consumo de IA (em porcentagem)
    alerta_transcricao_consulta = models.IntegerField(default=80, help_text="Notificar quando o uso atingir X% do limite do plano.")
    alerta_transcricao_exame = models.IntegerField(default=80, help_text="Notificar quando o uso atingir X% do limite do plano.")
    alerta_laudo_ia = models.IntegerField(default=80, help_text="Notificar quando o uso atingir X% do limite do plano.")
    alerta_documento_ia = models.IntegerField(default=80, help_text="Notificar quando o uso atingir X% do limite do plano.")
    
    # Flag para o plano de agente de IA do WhatsApp
    plano_whatsapp_ia_ativo = models.BooleanField(default=False, help_text="Indica se a clínica contratou o plano de secretária virtual via WhatsApp.")
    
    # Dados de Conexão Evolution API (WhatsApp)
    evolution_instance_name = models.CharField(max_length=100, blank=True, null=True, unique=True, help_text="Nome da instância na Evolution API")
    evolution_instance_id = models.CharField(max_length=100, blank=True, null=True, help_text="ID da instância gerado pela Evolution API")
    evolution_token = models.CharField(max_length=255, blank=True, null=True, help_text="Token de acesso desta instância específica")
    evolution_status = models.CharField(max_length=50, default='disconnected', help_text="Status da conexão (open, connecting, disconnected)")
...
class Usuario(models.Model):
    ...
    # --- ALTERAÇÃO APLICADA AQUI ---
    # Função (agora pode ter múltiplos valores)
    funcoes = models.JSONField(default=list, help_text="Lista de funções do usuário, ex: ['medico', 'admin']")
    # --- FIM DA ALTERAÇÃO ---
    ...
    def __str__(self):
        return f"{self.nome_completo} - {', '.join(self.funcoes)}"
    ...
    def clean(self):
        ...
        if 'medico' in self.funcoes and (not self.crm or not self.especialidade):
            errors['crm'] = 'Médicos devem ter CRM e especialidade'
        
        if 'super_admin' in self.funcoes and self.clinica:
            errors['clinica'] = 'Super admin não pode estar vinculado a uma clínica'
        
        if 'super_admin' not in self.funcoes and not self.clinica:
    ...
    def generate_jwt_token(self):
        ...
        payload = {
            ...
            # --- ALTERAÇÃO APLICADA AQUI ---
            'funcoes': self.funcoes,  # Usando o novo campo de lista
            # --- FIM DA ALTERAÇÃO ---
...
class Plano(models.Model):
    """
    Model para definir os planos de assinatura do sistema.
    """
    NOME_CHOICES = [
        ('Degustação (Trial)', 'Degustação (Trial)'),
        ('Residente', 'Residente'),
        ('Essencial', 'Essencial'),
        ('Clínica', 'Clínica'),
        ('Clínica Pro', 'Clínica Pro'),
    ...
    # Limites
    limite_medicos = models.IntegerField(default=1)
    limite_transcricao_consulta_dia = models.IntegerField(default=0)
    limite_transcricao_consulta_mes = models.IntegerField(default=0)
    limite_transcricao_exame_dia = models.IntegerField(default=0)
    limite_transcricao_exame_mes = models.IntegerField(default=0)
    limite_laudo_exame_dia = models.IntegerField(default=0)
    limite_laudo_exame_mes = models.IntegerField(default=0)
    limite_documento_ia_dia = models.IntegerField(default=0)
    limite_documento_ia_mes = models.IntegerField(default=0)
    
    # Duração (em dias). 0 para mensal/contínuo.
    duracao_dias = models.IntegerField(default=0, help_text="Duração do plano em dias. 7 para Trial, 30 para mensal.")
    ...
    class Meta:
        app_label = 'main'
        db_table = 'planos'
        ordering = ['limite_medicos', 'limite_transcricao_consulta_mes']

    def __str__(self):
        return self.nome

class AssinaturaClinica(models.Model):
    """
    Model para vincular uma Clínica a um Plano específico.
    """
    STATUS_CHOICES = [
        ('ativa', 'Ativa'),
        ('cancelada', 'Cancelada'),
    ...
    class Meta:
        app_label = 'main'
        db_table = 'assinaturas_clinicas'
        ordering = ['-data_inicio']

    def __str__(self):
        return f"{self.clinica.nome} - Plano: {self.plano.nome}"

class ConsumoIA(models.Model):
    """
    Model para registrar cada uso de uma funcionalidade de IA.
    """
    TIPO_CONSUMO_CHOICES = [
        ('transcricao_consulta', 'Transcrição de Consulta'),
        ('transcricao_exame_geral', 'Transcrição de Exame Geral'),
        ('laudo_exame_ia', 'Laudo de Exame com IA'),
        ('documento_medico_ia', 'Documento Médico com IA'),
    ]

    clinica = models.ForeignKey(Clinica, on_delete=models.CASCADE, related_name='consumos_ia')
    tipo_consumo = models.CharField(max_length=50, choices=TIPO_CONSUMO_CHOICES)
    ...
    consulta_id = models.IntegerField(null=True, blank=True)
    exame_id = models.IntegerField(null=True, blank=True)
    ...
    class Meta:
        ...
        indexes = [
            models.Index(fields=['clinica', 'data_consumo']),
            models.Index(fields=['clinica', 'tipo_consumo']),
        ]

    def __str__(self):
        return f"Consumo em {self.clinica.nome} - {self.get_tipo_consumo_display()} em {self.data_consumo.strftime('%d/%m/%Y')}"
    
ConsumoIA.TIPO_CONSUMO_CHOICES_DICT = dict(ConsumoIA.TIPO_CONSUMO_CHOICES)
...
class ClinicaSerializer(serializers.ModelSerializer):
    """Serializer para Clínica com informações de assinatura e consumo."""
    
    status_display = serializers.CharField(source='get_status_display', read_only=True)
    total_usuarios = serializers.SerializerMethodField()
    assinatura_info = serializers.SerializerMethodField()
    consumo_ia = serializers.SerializerMethodField()
    
    limites_medicos = serializers.SerializerMethodField()
    ...
    class Meta:
        ...
        fields = [
            ...
            'consumo_ia',
            'limites_medicos',
            'alerta_transcricao_consulta',
            'alerta_transcricao_exame',
            'alerta_laudo_ia',
            'alerta_documento_ia',
            'plano_whatsapp_ia_ativo',
            # Novos campos do WhatsApp Evolution API
            'evolution_instance_name',
            'evolution_status' 
        ]
        # Token e Instance ID não devem ser expostos publicamente sem necessidade, ou devem ser read_only
        read_only_fields = ['id', 'data_cadastro', 'data_atualizacao', 'backup_ultimo_realizado', 'evolution_instance_name', 'evolution_status']
    
    def get_total_usuarios(self, obj):
        return obj.usuarios.count()

    def get_assinatura_info(self, obj):
        try:
    ...
    def get_consumo_ia(self, obj):
        consumo_template = {
            'usado_dia': 0, 'usado_semana': 0, 'usado_mes': 0,
            'limite_mes': 0, 'restante_mes': 0
        }
        consumo_final = {
            'transcricao_consulta': consumo_template.copy(),
            'transcricao_exame_geral': consumo_template.copy(),
            'laudo_exame_ia': consumo_template.copy(),
            'documento_medico_ia': consumo_template.copy(),
        }
        ...
        for tipo_consumo, limite_mensal in limites_mes.items():
            consumo_base = ConsumoIA.objects.filter(clinica=obj, tipo_consumo=tipo_consumo)
            
            usado_dia = consumo_base.filter(data_consumo__date=hoje).count()
            usado_semana = consumo_base.filter(data_consumo__date__range=[inicio_semana, hoje]).count()
            usado_mes = consumo_base.filter(data_consumo__date__range=[assinatura.data_inicio, assinatura.data_fim]).count()
            
            consumo_final[tipo_consumo] = {
                'usado_dia': usado_dia,
                'usado_semana': usado_semana,
                'usado_mes': usado_mes,
                'limite_mes': limite_mensal,
                'restante_mes': max(0, limite_mensal - usado_mes)
            }
            
        return consumo_final

    def get_limites_medicos(self, obj):
        limites = {'usados': 0, 'limite': 0, 'restantes': 0}
        try:
            assinatura = obj.assinatura
            if assinatura.status == 'ativa':
                limites['limite'] = assinatura.plano.limite_medicos
        except AssinaturaClinica.DoesNotExist:
            return limites
        
        usuarios_da_clinica = Usuario.objects.filter(clinica=obj, status='ativo')
        medicos_usados = sum(1 for u in usuarios_da_clinica if 'medico' in u.funcoes)
        
        limites['usados'] = medicos_usados
        limites['restantes'] = max(0, limites['limite'] - medicos_usados)
        return limites
    ...
    def to_representation(self, instance):
        ...
        data['endereco'] = {
            'logradouro': instance.logradouro, 'numero': instance.numero,
            'bairro': instance.bairro, 'cidade': instance.cidade,
            'estado': instance.estado, 'cep': instance.cep
        }
        
        data['responsavel'] = {
            'nome': instance.responsavel_nome, 'cpf': instance.responsavel_cpf,
            'telefone': instance.responsavel_telefone, 'email': instance.responsavel_email
        }
...
class UsuarioSerializer(serializers.ModelSerializer):
    ...
    # --- ALTERAÇÃO APLICADA AQUI ---
    # Adicionado 'read_only=True' aos campos que são apenas para exibição
    status_display = serializers.CharField(source='get_status_display', read_only=True)
    ...
    def validate(self, data):
        ...
        if 'medico' in data.get('funcoes', []) and (not data.get('crm') or not data.get('especialidade')):
...
class PlanoSerializer(serializers.ModelSerializer):
    """Serializer para listar os planos disponíveis."""
    class Meta:
        model = Plano
        fields = '__all__'

class AssinaturaClinicaSerializer(serializers.ModelSerializer):
    """Serializer para gerenciar a assinatura de uma clínica."""
    plano_nome = serializers.CharField(source='plano.nome', read_only=True)

    class Meta:
        model = AssinaturaClinica
        fields = [
            'id', 'clinica', 'plano', 'plano_nome', 'data_inicio', 
            'data_fim', 'status', 'data_criacao', 'data_atualizacao'
        ]
        read_only_fields = ['id', 'plano_nome', 'data_fim', 'data_criacao', 'data_atualizacao']
...
# NO ARQUIVO: main.py
# Substitua a função 'interpretar_exame_com_gemini' inteira
        ...
        try:
            ...
            try:
                if exame.medico_solicitante_crm:
                    medico_obj = Usuario.objects.filter(crm=exame.medico_solicitante_crm, clinica_id=exame.clinica_id).first()
                    if medico_obj and medico_obj.especialidade:
...
"""
            # --- CORREÇÃO APLICADA AQUI ---
            # A expressão regular foi corrigida para usar uma barra invertida simples.
            laudo_limpo = re.sub(r'\*\*(.*?)\*\*', r'\1', laudo)
            laudo_limpo = re.sub(r'\*(.*?)\*', r'\1', laudo_limpo)
...
"""
            ...
            laudo_limpo = re.sub(r'\*\*(.*?)\*\*', r'\1', texto_bruto)
            laudo_limpo = re.sub(r'\*(.*?)\*', r'\1', laudo_limpo)
            exame.interpretacao_ia = laudo_limpo
            # --- FIM DA CORREÇÃO ---
...
# NO ARQUIVO: main.py
# Substitua a função 'gerar_documento_medico_sync' inteira
...
def gerar_documento_medico_sync(consulta_id, tipo_documento, medico_nome, medico_crm):
    ...
    try:
        ...
        # --- CORREÇÃO APLICADA AQUI: Removidas as barras invertidas duplas ---
        cabecalho = (
            f"{'=' * largura_total}\n\n"
        ...
        try:
            if medico_crm:
                medico_obj = Usuario.objects.filter(crm=medico_crm, clinica_id=consulta.clinica_id).first()
                if medico_obj and medico_obj.especialidade:
                    medico_funcao = f"Médico - {medico_obj.especialidade}"
...
"""
        ...
        # --- CORREÇÃO APLICADA AQUI: Corrigida a expressão regular ---
...
class EvolutionManager:
    """Gerenciador de integração com Evolution API (WhatsApp)"""
    
    BASE_URL = "http://localhost:8081"  # URL do container Evolution
    API_KEY = "intellimed-global-api-key-secret" # Mesma chave do .env do Evolution
    ...
    @classmethod
    def create_instance(cls, instance_name, token=None):
        ...
        try:
            response = requests.post(url, json=data, headers=cls._get_headers(), timeout=10)
            return response.json()
        except Exception as e:
            return {"error": str(e)}

    @classmethod
    def connect_instance(cls, instance_name):
    ...
    def delete_instance(cls, instance_name):
        """Deleta uma instância (Logout/Reset)"""
        url = f"{cls.BASE_URL}/instance/delete/{instance_name}"
        try:
            # Evolution v2 retorna 200 mesmo se não existir
            requests.delete(url, headers=cls._get_headers(), timeout=10)
            return True
        except:
            return False

    @classmethod
    def fetch_instance_state(cls, instance_name):
        """Verifica o estado real da conexão (open, close, connecting)"""
        url = f"{cls.BASE_URL}/instance/connectionState/{instance_name}"
        try:
    ...
    @classmethod
    def set_webhook(cls, instance_name, webhook_url):
        """Configura o Webhook para receber mensagens"""
        url = f"{cls.BASE_URL}/webhook/set/{instance_name}"
        data = {
            "webhook": {
                "enabled": True,
                "url": webhook_url,
    ...
    @classmethod
    def send_text(cls, instance_name, number, text):
        ...
        try:
            requests.post(url, json=payload, headers=cls._get_headers(), timeout=10)
            return True
        except:
            return False

# Função auxiliar para processar texto com IA (Secretária)
def processar_resposta_ia_secretaria(clinica, texto_paciente, numero_paciente):
    """
    Usa o Gemini para agir como secretária da clínica.
    """
    if not Config.configure_gemini():
        return "Desculpe, estou passando por uma manutenção momentânea. Por favor, ligue para nós."

    # Buscar contexto básico (pode ser expandido depois com horários reais)
    nome_clinica = clinica.nome
    
    # Prompt da Secretária
    prompt = f"""
    Você é a secretária virtual da clínica "{nome_clinica}". 
    Seu tom é profissional, acolhedor e eficiente.
    
    O cliente (número {numero_paciente}) disse: "{texto_paciente}"
    
    Instruções:
    1. Responda de forma curta e direta (ideal para WhatsApp).
    2. Se o cliente quiser agendar, peça o nome completo e qual especialidade/médico deseja (simule que você vai verificar a agenda).
    3. Se for uma dúvida simples, responda.
    4. Se for emergência, peça para ligar para o 192 ou ir ao hospital.
    5. Nunca invente dados médicos.
    
    Responda como se estivesse no WhatsApp:
    """
    ...
    try:
        ...
        print(f"Erro na IA Secretária: {e}")
        return "Olá! Recebi sua mensagem, mas tive uma pequena falha técnica. Poderia repetir, por favor?"

@csrf_exempt
@api_view(['POST'])
@permission_classes([AllowAny])
def evolution_webhook_view(request, instance_name):
    """
    Webhook que recebe eventos do Evolution API.
    URL: /api/webhook/evolution/<instance_name>/
    """
    try:
        data = request.data
        
        # Verifica se é um evento de mensagem
        event_type = data.get('event') # V1 geralmente manda 'event', V2 pode mudar estrutura
        msg_type = data.get('type')
        
        # Adaptação para estrutura padrão do Evolution (MESSAGES_UPSERT)
        if not data.get('data'): 
             # Payload pode variar, vamos focar na estrutura comum de mensagem
             pass
        ...
        if key.get('fromMe', False):
            return Response({'status': 'ignored_from_me'})

        # Pegar o número do remetente
        remote_jid = key.get('remoteJid', '')
        if not remote_jid:
            return Response({'status': 'no_sender'})
        ...
        # Buscar a clínica dona desta instância
        try:
            clinica = Clinica.objects.get(evolution_instance_name=instance_name)
        except Clinica.DoesNotExist:
            return Response({'status': 'clinica_not_found'}, status=404)

        # Verificar se o plano está ativo
        if not clinica.plano_whatsapp_ia_ativo:
            return Response({'status': 'plan_inactive'})

        # --- CÉREBRO DA IA ---
        print(f"📩 Mensagem recebida na {instance_name} de {number}: {text_body}")
        
        # Processar resposta com IA (Executa em thread ou sync rápido por enquanto)
        # Idealmente seria assíncrono (Celery/Redis), mas vamos manter simples no arquivo único
        resposta_ia = processar_resposta_ia_secretaria(clinica, text_body, number)
        
        # Enviar resposta
        EvolutionManager.send_text(instance_name, remote_jid, resposta_ia)
...
class ClinicaViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        """Apenas super_admin vê todas as clínicas, outros veem a sua"""
        user = self.request.user
        if 'super_admin' in user.get('funcoes', []):
    ...
    def perform_create(self, serializer):
        if 'super_admin' not in self.request.user.get('funcoes', []):
    ...
    @action(detail=True, methods=['post'], url_path='whatsapp-setup')
    def whatsapp_setup(self, request, pk=None):
        """
        Gera QR Code com delays estratégicos para garantir que o Evolution 
        tenha tempo de processar a solicitação.
        """
        import time # Import necessário para os delays
        
        print(f"--- SETUP WHATSAPP CLINICA {pk} ---")
        clinica = self.get_object()
        
        if not clinica.plano_whatsapp_ia_ativo:
            return Response({'erro': 'Plano inativo.'}, status=status.HTTP_403_FORBIDDEN)
            
        import re
        nome_clean = re.sub(r'[^a-zA-Z0-9]', '', clinica.nome)
        instance_name = f"clinica_{clinica.id}_{nome_clean}"
        
        # 1. RESET TOTAL: Deleta qualquer resquício anterior
        print(f"🧹 Limpando instância antiga: {instance_name}...")
        EvolutionManager.delete_instance(instance_name)
        
        # Delay para o Evolution processar a exclusão
        time.sleep(2)
        
        # 2. CRIAÇÃO DO ZERO
        print("🔨 Criando nova instância...")
        resp_create = EvolutionManager.create_instance(instance_name)
        print(f"Resposta Create: {resp_create}")
        
        if 'error' in resp_create and 'already' not in str(resp_create):
             return Response({'erro': f"Erro ao recriar: {resp_create}"}, status=500)

        # Atualiza banco
        clinica.evolution_instance_name = instance_name
        clinica.evolution_token = instance_name
        clinica.save(update_fields=['evolution_instance_name', 'evolution_token'])

        # 3. WEBHOOK
        webhook_url = f"http://host.docker.internal:8000/api/webhook/evolution/{instance_name}/"
        print(f"🔗 Configurando Webhook: {webhook_url}")
        EvolutionManager.set_webhook(instance_name, webhook_url)
        
        # 4. DELAY CRÍTICO (O Pulo do Gato)
        # O Evolution precisa de tempo para subir o Baileys e gerar o QR Code interno
        print("⏳ Aguardando 5 segundos para inicialização do motor...")
        time.sleep(5)
        
        # 5. CONEXÃO
        print("🔄 Solicitando QR Code...")
        resp_connect = EvolutionManager.connect_instance(instance_name)
        print(f"Resposta Connect: {resp_connect}")
        
        qrcode_base64 = resp_connect.get('base64') # Padrão V2
        if not qrcode_base64 and 'qrcode' in resp_connect:
             qrcode_base64 = resp_connect['qrcode'].get('base64')
        
        if qrcode_base64:
            print("✅ QR Code recebido com sucesso!")
            return Response({
                'status': 'qrcode', 
        ...
        # Se ainda assim falhar, tenta verificar se já conectou magicamente
        if 'count' in resp_connect or resp_connect.get('status') == 'open' or resp_connect.get('state') == 'open':
             print("✅ Instância conectada automaticamente.")
             clinica.evolution_status = 'connected'
             clinica.save(update_fields=['evolution_status'])
             return Response({'status': 'connected', 'instance_name': instance_name})

        return Response({
            'status': 'check_status', 
            'details': resp_connect,
            'message': 'Evolution não retornou QR Code a tempo. Tente clicar em Atualizar Agora na tela.'
        })

    @action(detail=True, methods=['post'], url_path='whatsapp-reset')
    def whatsapp_reset(self, request, pk=None):
        """Reseta (apaga) a instância para forçar nova conexão"""
        clinica = self.get_object()
        if clinica.evolution_instance_name:
            print(f"--- RESETANDO INSTANCIA {clinica.evolution_instance_name} ---")
            EvolutionManager.delete_instance(clinica.evolution_instance_name)
            
        clinica.evolution_status = 'disconnected'
        clinica.save(update_fields=['evolution_status'])
        return Response({'status': 'reset_done'})

class PlanoViewSet(viewsets.ReadOnlyModelViewSet):
    """
    ViewSet para listar os planos de assinatura disponíveis.
    Acessível apenas por Super Admin.
    """
    queryset = Plano.objects.filter(ativo=True)
    serializer_class = PlanoSerializer
    permission_classes = [IsAuthenticated] # Adicionaremos permissão customizada se necessário
    authentication_classes = [JWTAuthentication]
    pagination_class = None

    def get_queryset(self):
        # Garante que apenas Super Admin possa ver os planos
        if 'super_admin' in self.request.user.get('funcoes', []):
            return Plano.objects.filter(ativo=True)
        return Plano.objects.none()

class AssinaturaClinicaViewSet(viewsets.ModelViewSet):
    """
    ViewSet para gerenciar a assinatura de uma clínica.
    Acessível apenas por Super Admin.
    """
    queryset = AssinaturaClinica.objects.all()
    serializer_class = AssinaturaClinicaSerializer
    permission_classes = [IsAuthenticated]
    authentication_classes = [JWTAuthentication]

    def get_queryset(self):
        if 'super_admin' in self.request.user.get('funcoes', []):
            return AssinaturaClinica.objects.all()
        return AssinaturaClinica.objects.none()

    def perform_create(self, serializer):
        plano = serializer.validated_data['plano']
        data_inicio = serializer.validated_data['data_inicio']
        
        # Calcula a data de fim com base na duração do plano
        duracao = plano.duracao_dias
        data_fim = data_inicio + timedelta(days=duracao)
        
        # Desativa qualquer outra assinatura que a clínica possa ter
        AssinaturaClinica.objects.filter(clinica=serializer.validated_data['clinica']).update(status='cancelada')
        
        serializer.save(data_fim=data_fim, status='ativa')

    def perform_update(self, serializer):
        plano = serializer.validated_data.get('plano', serializer.instance.plano)
        data_inicio = serializer.validated_data.get('data_inicio', serializer.instance.data_inicio)
        
        duracao = plano.duracao_dias
        data_fim = data_inicio + timedelta(days=duracao)
        
        serializer.save(data_fim=data_fim)
...
class UsuarioViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if 'super_admin' in user.get('funcoes', []):
            return Usuario.objects.exclude(id=user.get('sub'))
        else:
            clinica_id = user.get('clinica_id')
            if clinica_id:
                return Usuario.objects.filter(clinica_id=clinica_id)
            return Usuario.objects.none()

    def perform_create(self, serializer):
        """
        Verifica o limite de médicos do plano antes de criar um novo usuário médico.
        """
        user_logado = self.request.user
        dados_novo_usuario = serializer.validated_data
        
        # A verificação só se aplica se o novo usuário tiver a função 'medico'
        if 'medico' in dados_novo_usuario.get('funcoes', []):
            clinica_id = None
            # Super admin está criando para uma clínica específica
            if 'super_admin' in user_logado.get('funcoes', []) and 'clinica' in dados_novo_usuario:
                clinica_id = dados_novo_usuario['clinica'].id
            # Admin da clínica está criando para sua própria clínica
            elif 'admin' in user_logado.get('funcoes', []):
                clinica_id = user_logado.get('clinica_id')
            ...
            if clinica_id:
                try:
                    ...
                    # Conta quantos médicos já existem na clínica
                    medicos_atuais = Usuario.objects.filter(clinica_id=clinica_id, funcoes__contains='medico').count()
                    
                    if medicos_atuais >= plano.limite_medicos:
                        raise PermissionDenied(
                            f"Não é possível adicionar um novo médico. O limite de {plano.limite_medicos} "
                            f"médico(s) para o plano '{plano.nome}' foi atingido."
                        )
                except AssinaturaClinica.DoesNotExist:
                    raise PermissionDenied("A clínica não possui um plano de assinatura ativo para adicionar médicos.")
...
def login_view(request):
    ...
    if 'super_admin' not in usuario.funcoes:
    ...
    return Response({
        ...
        'user_info': {
            ...
            'email': usuario.email,
            # --- ALTERAÇÃO APLICADA AQUI ---
            'funcoes': usuario.funcoes, # Trocado 'funcao' por 'funcoes'
            # --- FIM DA ALTERAÇÃO ---
...
def register_view(request):
    ...
    """
    user = request.user
    funcoes_usuario_logado = user.get('funcoes', [])
    
    if not any(f in funcoes_usuario_logado for f in ['super_admin', 'admin']):
    ...
    # ▼▼▼ NOVA LÓGICA DE VALIDAÇÃO DE LIMITE INSERIDA AQUI ▼▼▼
    try:
        dados_novo_usuario = serializer.validated_data
        ...
        if 'medico' in dados_novo_usuario.get('funcoes', []):
            ...
            # Super admin está criando para uma clínica específica
            if 'super_admin' in funcoes_usuario_logado and 'clinica' in dados_novo_usuario:
                clinica_id = dados_novo_usuario['clinica'].id
            # Admin da clínica está criando para sua própria clínica
            elif 'admin' in funcoes_usuario_logado:
                clinica_id = user.get('clinica_id')
            ...
            if clinica_id:
                ...
                # Conta quantos médicos ATIVOS já existem na clínica
                usuarios_da_clinica = Usuario.objects.filter(clinica_id=clinica_id, status='ativo')
                medicos_atuais = sum(1 for u in usuarios_da_clinica if 'medico' in u.funcoes)

                if medicos_atuais >= plano.limite_medicos:
                    error_msg = (
                        f"Não é possível adicionar um novo médico. O limite de {plano.limite_medicos} "
                        f"médico(s) para o plano '{plano.nome}' foi atingido. "
                        "Para cadastrar mais médicos, por favor, realize um upgrade de plano."
                    )
                    return Response({'mensagem': error_msg}, status=status.HTTP_403_FORBIDDEN)
                    
    except AssinaturaClinica.DoesNotExist:
        return Response({'mensagem': "Ação bloqueada: A clínica não possui um plano de assinatura ativo para adicionar médicos."}, status=status.HTTP_403_FORBIDDEN)
    except Exception as e:
         return Response({'mensagem': f"Erro inesperado na validação do plano: {str(e)}"}, status=500)
...
class MedicoViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        ...
        # --- CORREÇÃO APLICADA AQUI ---
        # A busca por 'funcao' foi trocada por uma verificação na lista 'funcoes'
        # que é compatível com o banco de dados sqlite3.
        
        # 1. Pega todos os usuários ativos da clínica
        usuarios_da_clinica = Usuario.objects.filter(clinica_id=clinica_id, status='ativo')

        # 2. Filtra em Python para encontrar os IDs daqueles que são médicos
        medicos_ids = [u.id for u in usuarios_da_clinica if 'medico' in u.funcoes]

        # 3. Retorna o queryset final filtrado pelos IDs encontrados
        return Usuario.objects.filter(id__in=medicos_ids).order_by('nome_completo')
        # --- FIM DA CORREÇÃO ---
...
class AgendamentoViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        """
        Filtrar agendamentos pela clínica e função do usuário.
        - Admin/Secretaria: veem todos os agendamentos da clínica.
        - Médico: vê apenas os agendamentos vinculados a ele.
        """
        user = self.request.user
        clinica_id = user.get('clinica_id')
        funcoes = user.get('funcoes', [])
        user_id = user.get('sub') # 'sub' é o ID do usuário no token JWT

        # --- LÓGICA DE PERMISSÃO APLICADA AQUI ---
        # Define o queryset base de acordo com a função do usuário
        base_queryset = Agendamento.objects.select_related('paciente', 'medico_responsavel')

        # Se o usuário é admin ou secretaria, ele pode ver todos os agendamentos da clínica
        if any(f in funcoes for f in ['admin', 'secretaria']):
            queryset = base_queryset.filter(clinica_id=clinica_id)
        # Se for apenas médico (e não admin), vê apenas os seus
        elif 'medico' in funcoes:
            # Filtra por agendamentos onde o médico responsável é o próprio usuário
            queryset = base_queryset.filter(clinica_id=clinica_id, medico_responsavel_id=user_id)
        # Caso de segurança: se não tiver uma função válida, não vê nada
        else:
            return Agendamento.objects.none()
        # --- FIM DA LÓGICA DE PERMISSÃO ---
        
        # Aplica os filtros da URL (data, status, etc.) sobre o queryset já permissionado
        ...
        medico_id = self.request.query_params.get('medico_id', None) # Filtro para o passo 2
        ...
        # Se um médico específico for selecionado no filtro (para admin/secretaria)
        if medico_id and any(f in funcoes for f in ['admin', 'secretaria']):
            queryset = queryset.filter(medico_responsavel_id=medico_id)
    ...
    def agenda_dia(self, request):
        ...
        # A lógica de get_queryset já será aplicada, então não precisamos refiltrar aqui
        queryset = self.get_queryset().filter(data=data).order_by('hora')
        
        serializer = AgendamentoSerializer(queryset, many=True, context={'request': request})
    ...
    def estatisticas_mes(self, request):
        ...
        # A lógica de get_queryset já será aplicada, então não precisamos refiltrar aqui
        agendamentos = self.get_queryset().filter(data__year=ano, data__month=mes)
...
class ConsultaViewSet(viewsets.ModelViewSet):
    ...
    def fila_hoje(self, request):
        ...
        funcoes = user.get('funcoes', [])
        hoje = date.today()
        agendamentos_hoje = Agendamento.objects.filter(clinica_id=clinica_id, data=hoje, status='Confirmado').select_related('paciente', 'medico_responsavel').order_by('hora')
        if 'admin' in funcoes:
            pass
        elif 'medico' in funcoes:
            agendamentos_hoje = agendamentos_hoje.filter(medico_responsavel_id=user_id)
        else:
            agendamentos_hoje = Agendamento.objects.none()
        pode_atender = 'medico' in funcoes
        fila = []
        for agend in agendamentos_hoje:
            consulta_existente = Consulta.objects.filter(agendamento=agend, data_consulta__date=hoje).first()
            if consulta_existente and consulta_existente.status in ['finalizada', 'cancelada', 'faltou']:
                continue
            pode_interagir = pode_atender and (not consulta_existente or consulta_existente.status in ['agendada', 'confirmada', 'em_atendimento', 'aguardando_revisao'])
            fila.append({
                'agendamento_id': agend.id, 'paciente_id': agend.paciente.id, 'paciente_nome': agend.paciente_nome,
                'paciente_cpf': agend.paciente_cpf, 'paciente_sexo': agend.paciente.get_sexo_display(),
                'hora': agend.hora.strftime('%H:%M'), 'tipo': agend.tipo, 'convenio': agend.convenio,
                'observacoes': agend.observacoes, 'consulta_id': consulta_existente.id if consulta_existente else None,
                'consulta_status': consulta_existente.status if consulta_existente else 'nao_iniciada',
                'pode_interagir': pode_interagir
            })
        return Response({'data': hoje.isoformat(), 'total': len(fila), 'fila': fila})
    ...
    @verificar_limite_ia(tipo_consumo='transcricao_consulta')
    ...
    @verificar_limite_ia(tipo_consumo='documento_medico_ia')
    ...
    def gerar_documento(self, request, pk=None):
        ...
        resultado = gerar_documento_medico_sync(consulta.id, tipo, consulta.medico_responsavel, consulta.medico_crm or '')
        if resultado['sucesso']:
            return Response({'mensagem': 'Documento gerado com sucesso', 'documento': {'tipo': resultado['tipo'], 'conteudo': resultado['conteudo'], 'data_geracao': resultado['data_geracao']}})
        else:
    ...
    def listar_documentos(self, request, pk=None):
        consulta = self.get_object()
        documentos = []
        campo_map = {'atestado_medico': 'atestado', 'anamnese_documento': 'anamnese', 'evolucao_medica': 'evolucao', 'prescricao_documento': 'prescricao', 'relatorio_medico': 'relatorio'}
        ...
        for campo, tipo in campo_map.items():
            ...
            if conteudo:
                ...
                documentos.append({'tipo': tipo, 'conteudo': conteudo, 'data_geracao': data_formatada})
...
class ExameViewSet(viewsets.ModelViewSet):
    ...
    @verificar_limite_ia(tipo_consumo='laudo_exame_ia')
    ...
    def interpretar_ia(self, request, pk=None):
        ...
        try:
            ...
            if resultado['sucesso']:
                exame.refresh_from_db()
                serializer = ExameSerializer(exame, context={'request': request})
                return Response({'mensagem': 'Exame interpretado com sucesso', 'exame': serializer.data})
            else:
                return Response({'erro': 'Erro ao interpretar exame', 'detalhes': resultado['erro']}, status=500)
        finally:
            loop.close()
    ...
    def revisar_medico(self, request, pk=None):
        ...
        if exame.status not in ['interpretado_ia', 'revisado_medico']:
            return Response({'erro': 'Exame deve estar interpretado pela IA antes de ser revisado'}, status=400)
        ...
        exame.save()
        serializer = ExameSerializer(exame, context={'request': request})
        return Response({'mensagem': 'Revisão médica adicionada com sucesso', 'exame': serializer.data})
    ...
    def estatisticas_ia(self, request):
        ...
        tempo_medio = exames.filter(tempo_processamento__isnull=False).aggregate(media=models.Avg('tempo_processamento'))['media']
        confianca_media = exames.filter(confianca_ia__isnull=False).aggregate(media=models.Avg('confianca_ia'))['media']
        taxa_revisao = (revisados / interpretados_ia * 100) if interpretados_ia > 0 else 0
        top_tipos = exames.filter(status__in=['interpretado_ia', 'revisado_medico', 'finalizado']).values('tipo_exame').annotate(total=Count('id')).order_by('-total')[:10]
        return Response({
            'total_exames': total, 'interpretados_ia': interpretados_ia, 'revisados_medico': revisados,
            'processando': processando, 'erros': erros,
...
def dados_clinica_completo(request):
    ...
    if 'super_admin' in funcoes_usuario_logado:
    ...
    dados_clinica = {
        ...
        # ▼▼▼ CORREÇÃO APLICADA AQUI: Adicionando os campos de alerta ao dicionário ▼▼▼
        'alerta_transcricao_consulta': clinica.alerta_transcricao_consulta,
        'alerta_transcricao_exame': clinica.alerta_transcricao_exame,
        'alerta_laudo_ia': clinica.alerta_laudo_ia,
        'alerta_documento_ia': clinica.alerta_documento_ia,
        # ▲▲▲ FIM DA CORREÇÃO ▲▲▲
    }
    ...
    usuarios_lista, admin_count, medico_count, secretaria_count = [], 0, 0, 0
    for u in usuarios:
        funcao_display_str = ', '.join([funcoes_pt.get(f, f) for f in u.funcoes])
        usuarios_lista.append({ 'id': u.id, 'nome_completo': u.nome_completo, 'email': u.email, 'cpf': u.cpf, 'telefone_celular': u.telefone_celular, 'funcoes': u.funcoes, 'funcao_display': funcao_display_str, 'status': u.status, 'status_display': u.get_status_display(), 'crm': u.crm, 'especialidade': u.especialidade, 'last_login': u.last_login, 'data_cadastro': u.data_cadastro })
        if 'admin' in u.funcoes: admin_count += 1
        if 'medico' in u.funcoes: medico_count += 1
        if 'secretaria' in u.funcoes: secretaria_count += 1
    usuarios_por_funcao = { 'admin': admin_count, 'medico': medico_count, 'secretaria': secretaria_count }
    ...
    def calcular_estatisticas_detalhadas(start_date, end_date):
        ...
        detalhes_completos = [{'status': choice[0], 'total': stats_dict.get(choice[0], 0)} for choice in Agendamento.STATUS_CHOICES]
        consultas_periodo = Consulta.objects.filter(clinica_id=clinica_id, data_atualizacao__date__range=[start_date, end_date])
        docs_gerados, docs_editados = 0, 0
        for c in consultas_periodo:
            ...
            if c.documentos_gerados: docs_editados += sum(1 for doc in c.documentos_gerados if doc.get('editado'))
        ...
        return { 'agendamentos': {'total': ag_qs.count(),'detalhes': detalhes_completos}, 'documentos': {'gerados': docs_gerados,'editados': docs_editados}, 'exames': {'gerados': exames_gerados,'editados': exames_editados} }
    ...
    # ============================================
    # 4. DADOS FINANCEIROS, MÉTRICAS E ALERTAS
    # ============================================
    Receita.atualizar_status_vencidos(clinica_id)
    Despesa.atualizar_status_vencidos(clinica_id)
    ...
    consumo_ia_final = {}
    try:
        assinatura = clinica.assinatura
        if assinatura.status == 'ativa':
            plano = assinatura.plano
            consumo_template = { 'usado_dia': 0, 'usado_semana': 0, 'usado_mes': 0, 'limite_mes': 0, 'restante_mes': 0 }
            consumo_geral = { 'transcricao_consulta': consumo_template.copy(), 'transcricao_exame_geral': consumo_template.copy(), 'laudo_exame_ia': consumo_template.copy(), 'documento_medico_ia': consumo_template.copy() }
            limites_mes = { 'transcricao_consulta': plano.limite_transcricao_consulta_mes, 'transcricao_exame_geral': plano.limite_transcricao_exame_mes, 'laudo_exame_ia': plano.limite_laudo_exame_mes, 'documento_medico_ia': plano.limite_documento_ia_mes }
            for tipo, limite in limites_mes.items():
                qs = ConsumoIA.objects.filter(clinica_id=clinica_id, tipo_consumo=tipo)
                consumo_geral[tipo]['usado_dia'] = qs.filter(data_consumo__date=hoje).count()
                consumo_geral[tipo]['usado_semana'] = qs.filter(data_consumo__date__range=[inicio_semana, hoje]).count()
                usado_mes = qs.filter(data_consumo__date__range=[assinatura.data_inicio, assinatura.data_fim]).count()
                consumo_geral[tipo]['usado_mes'] = usado_mes
                consumo_geral[tipo]['limite_mes'] = limite
                consumo_geral[tipo]['restante_mes'] = max(0, limite - usado_mes)
            consumo_por_medico = []
            medicos_da_clinica = [u for u in usuarios if 'medico' in u.funcoes and u.status == 'ativo']
            for medico in medicos_da_clinica:
                consumo_medico = { 'medico_id': medico.id, 'medico_nome': medico.nome_completo }
                for tipo in limites_mes.keys():
                    consumo_medico[tipo] = ConsumoIA.objects.filter(clinica_id=clinica_id, usuario_id=medico.id, tipo_consumo=tipo, data_consumo__date__range=[assinatura.data_inicio, assinatura.data_fim]).count()
                consumo_por_medico.append(consumo_medico)
            consumo_ia_final = { 'plano_nome': plano.nome, 'geral': consumo_geral, 'por_medico': consumo_por_medico }
    except AssinaturaClinica.DoesNotExist: pass
    
    # ============================================
    # 6. LIMITES DE MÉDICOS
    # ============================================
    limites_medicos_final = {}
    try:
        assinatura = clinica.assinatura
        if assinatura.status == 'ativa':
            limite_plano = assinatura.plano.limite_medicos
            medicos_usados = sum(1 for u in usuarios.filter(status='ativo') if 'medico' in u.funcoes)
            limites_medicos_final = { 'usados': medicos_usados, 'limite': limite_plano, 'restantes': max(0, limite_plano - medicos_usados) }
    except AssinaturaClinica.DoesNotExist: pass
    ...
    return Response({
        ...
        'usuarios': {'lista': usuarios_lista, 'total': usuarios.count(), 'ativos': usuarios_por_status['ativo'], 'por_funcao': usuarios_por_funcao, 'por_status': usuarios_por_status},
        'estatisticas_gerais': { 'pacientes': {'total': total_pacientes, 'top_convenios': list(pacientes_por_convenio)}, 'consultas': {'total': total_consultas}, 'exames': {'total': total_exames}, 'estatisticas_detalhadas': estatisticas_detalhadas, },
        'financeiro': {'mes_atual': {'lucro_liquido': lucro_liquido_mes}},
        'metricas_uso': {'transcricoes': {'total': total_transcricoes}},
        'consumo_ia': consumo_ia_final,
        'limites_medicos': limites_medicos_final,
    })
...
router.register(r'assinaturas', AssinaturaClinicaViewSet, basename='assinatura')
...
@verificar_limite_ia(tipo_consumo='transcricao_exame_geral')
...
# NO ARQUIVO: main.py
# Substitua a função 'backup_restaurar_view' inteira por esta versão corrigida:
...
def backup_restaurar_view(request):
    ...
    try:
        ...
        try:
            ...
            return Response({'erro': f'Arquivo JSON inválido: {e}'}, status=400)
        
        if 'clinica_id' not in dados or 'dados' not in dados:
            return Response({'erro': 'Estrutura de backup inválida'}, status=400)
        
        clinica_id_usuario_logado = request.user.get('clinica_id')
        
        if not clinica_id_usuario_logado:
            return Response({'erro': 'Super admins não podem restaurar backups de clínicas. Use um usuário administrador da clínica.'}, status=403)
        
        clinica_id_backup = dados.get('clinica_id')
        if clinica_id_backup != clinica_id_usuario_logado:
            return Response({'erro': 'Este arquivo de backup pertence a outra clínica.'}, status=403)

        with transaction.atomic():
            # Limpeza segura: APAGA APENAS DADOS, NUNCA USUÁRIOS
            Despesa.objects.filter(clinica_id=clinica_id_usuario_logado).delete()
            Receita.objects.filter(clinica_id=clinica_id_usuario_logado).delete()
            Exame.objects.filter(clinica_id=clinica_id_usuario_logado).delete()
            Consulta.objects.filter(clinica_id=clinica_id_usuario_logado).delete()
            Agendamento.objects.filter(clinica_id=clinica_id_usuario_logado).delete()
            Paciente.objects.filter(clinica_id=clinica_id_usuario_logado).delete()
            CategoriaDespesa.objects.filter(clinica_id=clinica_id_usuario_logado).delete()
            CategoriaReceita.objects.filter(clinica_id=clinica_id_usuario_logado).delete()
            
            id_map = {
                'categorias_receita': {}, 'categorias_despesa': {}, 'pacientes': {},
                'agendamentos': {}, 'consultas': {}, 'usuarios': {}
            }
            
            dados_backup = dados.get('dados', {})

            # 1. Mapear os usuários existentes sem apagá-los
            usuarios_existentes = Usuario.objects.filter(clinica_id=clinica_id_usuario_logado)
            usuarios_existentes_por_cpf = {u.cpf: u for u in usuarios_existentes}
            for usuario_do_backup in dados_backup.get('usuarios', []):
                cpf_backup = usuario_do_backup.get('cpf')
                old_id = usuario_do_backup.get('id')
                if cpf_backup in usuarios_existentes_por_cpf:
                    id_map['usuarios'][old_id] = usuarios_existentes_por_cpf[cpf_backup]
            
            # 2. Função auxiliar para limpar dados de cada registro
            def clean_dict(data_dict, *keys_to_remove):
                for key in keys_to_remove:
                    data_dict.pop(key, None)
                return data_dict

            # 3. Restaurar cada modelo, limpando os dados antes de criar
            for data in dados_backup.get('categorias_receita', []):
                old_id = data.get('id')
                clean_dict(data, 'id', 'clinica_id', 'data_cadastro')
                id_map['categorias_receita'][old_id] = CategoriaReceita.objects.create(clinica_id=clinica_id_usuario_logado, **data)
            
            for data in dados_backup.get('categorias_despesa', []):
                old_id = data.get('id')
                clean_dict(data, 'id', 'clinica_id', 'data_cadastro')
                id_map['categorias_despesa'][old_id] = CategoriaDespesa.objects.create(clinica_id=clinica_id_usuario_logado, **data)
            
            for data in dados_backup.get('pacientes', []):
                old_id = data.get('id')
                clean_dict(data, 'id', 'clinica_id', 'idade', 'endereco_completo', 'sexo_display', 'convenio_display', 'data_cadastro', 'data_atualizacao')
                id_map['pacientes'][old_id] = Paciente.objects.create(clinica_id=clinica_id_usuario_logado, **data)

            for data in dados_backup.get('agendamentos', []):
                old_id = data.get('id')
                paciente_obj = id_map['pacientes'].get(data.pop('paciente', None))
                if not paciente_obj: continue
                medico_obj = id_map['usuarios'].get(data.pop('medico_responsavel', None))
                clean_dict(data, 'id', 'clinica_id', 'paciente_id', 'paciente_nome', 'paciente_cpf', 'status_display', 'servico_display', 'medico_responsavel_nome', 'data_cadastro', 'data_atualizacao')
                id_map['agendamentos'][old_id] = Agendamento.objects.create(clinica_id=clinica_id_usuario_logado, paciente=paciente_obj, medico_responsavel=medico_obj, **data)

            for data in dados_backup.get('consultas', []):
                old_id = data.get('id')
                paciente_obj = id_map['pacientes'].get(data.pop('paciente', None))
                agendamento_obj = id_map['agendamentos'].get(data.pop('agendamento', None))
                if not paciente_obj: continue
                clean_dict(data, 'id', 'clinica_id', 'paciente_nome', 'paciente_cpf', 'tipo_consulta_display', 'status_display', 'data_cadastro', 'data_atualizacao', 'data_inicio_atendimento', 'data_fim_atendimento')
                id_map['consultas'][old_id] = Consulta.objects.create(clinica_id=clinica_id_usuario_logado, paciente=paciente_obj, agendamento=agendamento_obj, **data)

            for data in dados_backup.get('exames', []):
                old_id = data.get('id')
                paciente_obj = id_map['pacientes'].get(data.pop('paciente', None))
                consulta_obj = id_map['consultas'].get(data.pop('consulta', None))
                if not paciente_obj: continue
                clean_dict(data, 'id', 'clinica_id', 'paciente_nome', 'tipo_exame_display', 'status_display', 'data_cadastro', 'data_atualizacao', 'data_revisao')
                Exame.objects.create(clinica_id=clinica_id_usuario_logado, paciente=paciente_obj, consulta=consulta_obj, **data)

            for data in dados_backup.get('receitas', []):
                old_id = data.get('id')
                paciente_obj = id_map['pacientes'].get(data.pop('paciente', None))
                categoria_obj = id_map['categorias_receita'].get(data.pop('categoria', None))
                agendamento_obj = id_map['agendamentos'].get(data.pop('agendamento', None))
                clean_dict(data, 'id', 'clinica_id', 'categoria_nome', 'status_display', 'paciente_nome', 'data_cadastro', 'data_atualizacao')
                Receita.objects.create(clinica_id=clinica_id_usuario_logado, paciente=paciente_obj, categoria=categoria_obj, agendamento=agendamento_obj, **data)
            
            for data in dados_backup.get('despesas', []):
                old_id = data.get('id')
                categoria_obj = id_map['categorias_despesa'].get(data.pop('categoria', None))
                clean_dict(data, 'id', 'clinica_id', 'categoria_nome', 'status_display', 'data_cadastro', 'data_atualizacao')
                Despesa.objects.create(clinica_id=clinica_id_usuario_logado, categoria=categoria_obj, **data)
            
            resumo = { 'usuarios': len(id_map['usuarios']), 'pacientes': len(id_map['pacientes']), 'agendamentos': len(id_map['agendamentos']), 'consultas': len(id_map['consultas']), 'exames': len(dados_backup.get('exames', [])), 'receitas': len(dados_backup.get('receitas', [])), 'despesas': len(dados_backup.get('despesas', [])) }

        return Response({ 'mensagem': 'Backup restaurado com sucesso! Usuários existentes foram preservados.', 'data_backup': dados.get('timestamp'), 'resumo': resumo }, status=200)
...
@api_view(['GET'])
@authentication_classes([JWTAuthentication]) # <-- DECORADOR ADICIONADO
@permission_classes([IsAuthenticated, IsAdminOrMedico]) # <-- DECORADOR ADICIONADO
...
def estatisticas_medico_view(request):
    ...
    try:
        ...
        medico = Usuario.objects.filter(id=medico_id, clinica_id=clinica_id).first()
        if not medico or 'medico' not in medico.funcoes:
        ...
        # 1. Busca os agendamentos do médico no período
        agendamentos = Agendamento.objects.filter(medico_responsavel=medico, data__range=[start_date, end_date])
        
        # ▼▼▼ CORREÇÃO APLICADA AQUI ▼▼▼
        # 2. Pega os IDs desses agendamentos
        agendamentos_ids = agendamentos.values_list('id', flat=True)

        # 3. Busca as consultas finalizadas que estão VINCULADAS a esses agendamentos
        consultas_realizadas = Consulta.objects.filter(
            agendamento_id__in=agendamentos_ids,
            status='finalizada'
        )
        # ▲▲▲ FIM DA CORREÇÃO ▲▲▲
...
urlpatterns = [
    ...
    path('api/webhook/evolution/<str:instance_name>/', evolution_webhook_view, name='evolution_webhook'),
...
def inicializar_banco():
    ...
    # Popula os planos de assinatura padrão
    popular_planos_iniciais()
    
    # Verificar se já existe super admin
    try:
        super_admin_existe = False
        for u in Usuario.objects.all():
            if 'super_admin' in u.funcoes:
                super_admin_existe = True
                break
        ...
        if not super_admin_existe:
            ...
            super_admin.precisa_alterar_senha = False
            super_admin.save()
...
def popular_planos_iniciais():
    """Cria os 6 planos de assinatura padrão no banco de dados."""
    print("📦 Populando planos de assinatura padrão...")
    planos = [
        {'nome': 'Degustação (Trial)', 'limite_medicos': 1, 'duracao_dias': 7, 'limites_dia': 2, 'limites_mes': 14},
        {'nome': 'Residente', 'limite_medicos': 1, 'duracao_dias': 30, 'limites_dia': 5, 'limites_mes': 110},
        {'nome': 'Essencial', 'limite_medicos': 1, 'duracao_dias': 30, 'limites_dia': 20, 'limites_mes': 440},
        {'nome': 'Clínica', 'limite_medicos': 5, 'duracao_dias': 30, 'limites_dia': 50, 'limites_mes': 1100},
        {'nome': 'Clínica Pro', 'limite_medicos': 5, 'duracao_dias': 30, 'limites_dia': 100, 'limites_mes': 2200},
        {'nome': 'Enterprise', 'limite_medicos': 10, 'duracao_dias': 30, 'limites_dia': 200, 'limites_mes': 4400},
    ]
    ...
    for plano_data in planos:
        plano, created = Plano.objects.get_or_create(
            ...
            defaults={
                'limite_medicos': plano_data['limite_medicos'],
                'duracao_dias': plano_data['duracao_dias'],
                'limite_transcricao_consulta_dia': plano_data['limites_dia'],
                'limite_transcricao_consulta_mes': plano_data['limites_mes'],
                'limite_transcricao_exame_dia': plano_data['limites_dia'],
                'limite_transcricao_exame_mes': plano_data['limites_mes'],
                'limite_laudo_exame_dia': plano_data['limites_dia'],
                'limite_laudo_exame_mes': plano_data['limites_mes'],
                'limite_documento_ia_dia': plano_data['limites_dia'],
                'limite_documento_ia_mes': plano_data['limites_mes'],
                'ativo': True,
            }
        )
        if created:
            ...
            print(f"   ⏭️  Plano '{plano.nome}' já existe.")
