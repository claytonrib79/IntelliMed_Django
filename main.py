...
"""
        ...
        if exame.arquivo_exame:
            if exame.arquivo_exame.startswith('data:'):
                ...
                base64_string = exame.arquivo_exame.split(',', 1)
...
"""
        ...
        if consulta.audio_consulta:
            if consulta.audio_consulta.startswith('data:'):
                # <<< AQUI ESTÁ A CORREÇÃO VERIFICADA >>>
                base64_string = consulta.audio_consulta.split(',', 1)
...
async def gerar_documento_medico(consulta_id, tipo_documento, medico_nome, medico_crm):
    ...
    try:
        ...
        genai.configure(api_key=Config.GEMINI_API_KEY)
        # <<< LINHA CORRIGIDA COM O MODELO QUE VOCÊ ESPECIFICOU >>>
        model = genai.GenerativeModel('gemini-2.5-pro')
...
class AgendamentoViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        # OTIMIZAÇÃO: .select_related('paciente') busca os dados do paciente na mesma consulta ao banco
    ...
    def alterar_status(self, request, pk=None):
        ...
        # <<< INÍCIO DA NOVA LÓGICA IMPLEMENTADA >>>
        
        status_antigo = agendamento.status
        
        # Se o status ANTERIOR era 'Realizado' e o NOVO status é DIFERENTE de 'Realizado',
        # significa que o usuário desfez a ação. Devemos apagar a receita associada.
        if status_antigo == 'Realizado' and novo_status != 'Realizado':
            # Filtra e deleta qualquer receita que tenha sido gerada por este agendamento
        ...
        # <<< FIM DA NOVA LÓGICA IMPLEMENTADA >>>
...
class ExameViewSet(viewsets.ModelViewSet):
    ...
    def upload_interpretar_ia(self, request):
        ...
        # <<< INÍCIO DA CORREÇÃO >>>
        # Chamada direta para a função agora síncrona
        try:
