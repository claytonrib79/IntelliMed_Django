...
"""
        ...
        try:
            ...
            # <<< INÍCIO DA ALTERAÇÃO NO TEXTO DO LAUDO >>>
            laudo = f"""
╔════════════════════════════════════════════════════════════════════════════════════════╗
...
═══════════════════════════════════════════════════════════════════════════════════════
"""
            # <<< FIM DA ALTERAÇÃO NO TEXTO DO LAUDO >>>
            ...
            print(f"Erro: {json_error}")
            print(f"String que falhou: >>>{json_str}<<<")
            print(f"--------------------------------")
...
def gerar_documento_medico_sync(consulta_id, tipo_documento, medico_nome, medico_crm):
    """
    Gera documento médico baseado na consulta (VERSÃO SÍNCRONA, ROBUSTA E RESTRITIVA)
    """
    try:
        # 1. VALIDAÇÃO E CONFIGURAÇÃO DA API (sempre no início)
        if not Config.GEMINI_API_KEY:
        ...
        # 3. CONSTRUÇÃO DOS PROMPTS RESTRITIVOS
        # Instrução principal, repetida em todos os prompts para reforçar.
        instrucao_principal = "ATENÇÃO: Use SOMENTE as informações fornecidas abaixo. NÃO INVENTE dados, valores de exames, nomes de medicamentos ou diagnósticos que não estejam explicitamente mencionados na transcrição ou nos dados da consulta. Seja objetivo e baseie-se exclusivamente no contexto fornecido."

        prompts = {
            'atestado': f"""
                {instrucao_principal}
                Gere um ATESTADO MÉDICO profissional.
                - Paciente: {paciente.nome_completo}
                - CPF: {paciente.cpf}
                - Data da Consulta: {data_extenso}
                - Hipótese Diagnóstica/Diagnóstico (use apenas o que foi fornecido): {consulta.diagnostico or consulta.hipotese_diagnostica or 'A ser esclarecido em consulta.'}
                - Transcrição da Consulta: {consulta.transcricao_ia}
                - Médico: {medico_nome} (CRM: {medico_crm})
                Se a transcrição mencionar dias de afastamento, inclua no atestado. Caso contrário, não mencione.
            """,
            'anamnese': f"""
                {instrucao_principal}
                Gere uma ANAMNESE ESTRUTURADA.
                - Paciente: {paciente.nome_completo}, {paciente.idade} anos.
                - Dados da Consulta (Use APENAS o que estiver preenchido):
                  - Queixa Principal: {consulta.queixa_principal}
                  - História da Doença Atual: {consulta.historia_doenca_atual}
                  - Exame Físico: {consulta.exame_fisico}
                - Transcrição Completa da Conversa: {consulta.transcricao_ia}
                Compile as informações nos seguintes tópicos, preenchendo apenas com dados existentes: Identificação, Queixa Principal, HDA, Antecedentes (se mencionado), Exame Físico (se mencionado), Hipóteses Diagnósticas.
            """,
            'evolucao': f"""
                {instrucao_principal}
                Gere uma EVOLUÇÃO MÉDICA no formato S.O.A.P.
                - Paciente: {paciente.nome_completo}
                - Data/Hora: {consulta.data_consulta.strftime('%d/%m/%Y %H:%M')}
                - Transcrição da Consulta: {consulta.transcricao_ia}
                - Dados Estruturados da Consulta:
                  - Queixa (Subjetivo): {consulta.queixa_principal}
                  - Exame Físico (Objetivo): {consulta.exame_fisico}
                  - Hipótese (Avaliação): {consulta.hipotese_diagnostica}
                  - Conduta (Plano): {consulta.conduta}
                Sintetize essas informações no formato S.O.A.P.
            """,
            'prescricao': f"""
                {instrucao_principal}
                Gere uma PRESCRIÇÃO MÉDICA.
                - Paciente: {paciente.nome_completo}
                - Data: {data_extenso}
                - Médico: {medico_nome} (CRM: {medico_crm})
                - Informações da Conduta e Medicamentos mencionados na consulta:
                  - Campo 'Conduta': {consulta.conduta}
                  - Transcrição da Consulta: {consulta.transcricao_ia}
                Liste APENAS os medicamentos explicitamente mencionados na conduta ou na transcrição, com dosagem e posologia se informados. Se nenhuma medicação for mencionada, escreva "Nenhuma medicação prescrita nesta consulta.".
            """,
            'relatorio': f"""
                {instrucao_principal}
                Gere um RELATÓRIO MÉDICO detalhado.
                - Paciente: {paciente.nome_completo}, CPF: {paciente.cpf}
                - Data da Consulta: {data_extenso}
                - Transcrição da Consulta: {consulta.transcricao_ia}
                - Dados Estruturados:
                  - Queixa Principal: {consulta.queixa_principal}
                  - HDA: {consulta.historia_doenca_atual}
                  - Exame Físico: {consulta.exame_fisico}
                  - Hipótese Diagnóstica: {consulta.hipotese_diagnostica}
                  - Conduta Proposta: {consulta.conduta}
                Compile todas essas informações em um relatório médico coeso e profissional, sem adicionar informações novas.
            """
        }
        ...
        if not response.text or not response.text.strip():
            raise Exception("A IA retornou uma resposta vazia ao tentar gerar o documento.")

        documento = response.text.replace('```', '')
...
class ConsultaViewSet(viewsets.ModelViewSet):
    ...
    def listar_documentos(self, request, pk=None):
        ...
        consulta = self.get_object()
        documentos = [] 
        if consulta.atestado_medico: documentos.append({'tipo': 'atestado', 'conteudo': consulta.atestado_medico})
        if consulta.anamnese_documento: documentos.append({'tipo': 'anamnese', 'conteudo': consulta.anamnese_documento})
        if consulta.evolucao_medica: documentos.append({'tipo': 'evolucao', 'conteudo': consulta.evolucao_medica})
        if consulta.prescricao_documento: documentos.append({'tipo': 'prescricao', 'conteudo': consulta.prescricao_documento})
        if consulta.relatorio_medico: documentos.append({'tipo': 'relatorio', 'conteudo': consulta.relatorio_medico})
...
class ExameViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if interpretado_ia is not None:
            # Filtra por exames que possuem o campo 'interpretacao_ia' preenchido
            if interpretado_ia.lower() == 'true':
                queryset = queryset.exclude(interpretacao_ia__isnull=True).exclude(interpretacao_ia__exact='')
            # Filtra por exames que NÃO possuem o campo 'interpretacao_ia' preenchido
...
urlpatterns = [
    ...
    path('api/transcrever-audio/', transcrever_audio_view, name='transcrever-audio'), # <<< ROTA ADICIONADA
