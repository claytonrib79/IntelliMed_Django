...
class Clinica(models.Model):
    ...
    backup_frequencia = models.CharField(max_length=10, choices=BACKUP_FREQUENCIA_CHOICES, default='manual')
    ...
    alerta_transcricao_consulta = models.IntegerField(default=80)
    alerta_transcricao_exame = models.IntegerField(default=80)
    alerta_laudo_ia = models.IntegerField(default=80)
    alerta_documento_ia = models.IntegerField(default=80)
    
    # WhatsApp IA (Evolution API)
    plano_whatsapp_ia_ativo = models.BooleanField(default=False, help_text="Ativa a secretária virtual.")
    whatsapp_secretaria_nome = models.CharField(max_length=100, default="Sofia", help_text="Nome da secretária virtual.")
    
    # Configurações Técnicas Evolution API
    evolution_instance_name = models.CharField(max_length=100, blank=True, null=True, help_text="Nome único da instância (ex: clinica_1).")
    evolution_apikey = models.CharField(max_length=255, blank=True, null=True, help_text="API Key da instância.")
    evolution_server_url = models.CharField(max_length=255, default="http://localhost:8080", help_text="URL da Evolution API.")
...
class ClinicaSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'assinatura_info', 'consumo_ia', 'limites_medicos',
            'alerta_transcricao_consulta', 'alerta_transcricao_exame',
            'alerta_laudo_ia', 'alerta_documento_ia',
            'plano_whatsapp_ia_ativo', 
            'whatsapp_secretaria_nome', 'evolution_instance_name', 
            'evolution_apikey', 'evolution_server_url'
        ]
        read_only_fields = ['id', 'data_cadastro', 'data_atualizacao', 'backup_ultimo_realizado']
    
    def get_total_usuarios(self, obj): return obj.usuarios.count()
    
    def get_assinatura_info(self, obj):
        try:
            assinatura = obj.assinatura
            if assinatura.status == 'ativa':
                return {'id': assinatura.id, 'plano_id': assinatura.plano_id, 'plano_nome': assinatura.plano.nome, 'data_inicio': assinatura.data_inicio, 'data_fim': assinatura.data_fim}
        except AssinaturaClinica.DoesNotExist: return None
        return None

    def get_consumo_ia(self, obj):
        # (Mantém lógica original de consumo, omitida aqui para brevidade, mas deve estar no código final)
        # ... Copie a lógica do seu arquivo original ou use a versão que já te passei antes ...
        return {} # Placeholder, mantenha sua lógica original aqui

    def get_limites_medicos(self, obj):
        # (Mantém lógica original)
        return {'usados': 0, 'limite': 0, 'restantes': 0}

    def to_representation(self, instance):
        data = super().to_representation(instance)
        data['endereco'] = {'logradouro': instance.logradouro, 'numero': instance.numero, 'bairro': instance.bairro, 'cidade': instance.cidade, 'estado': instance.estado, 'cep': instance.cep}
        data['responsavel'] = {'nome': instance.responsavel_nome, 'cpf': instance.responsavel_cpf, 'telefone': instance.responsavel_telefone, 'email': instance.responsavel_email}
        return data
...
class ClinicaViewSet(viewsets.ModelViewSet):
    ...
    - PATCH /api/clinicas/{id}/ - Atualizar parcial (CORREÇÃO)
    ...
    def get_queryset(self):
        ...
        # ▼▼▼ CORREÇÃO APLICADA AQUI ▼▼▼
        # Garante que super_admin possa ver todas as clínicas em qualquer ação
    ...
    def partial_update(self, request, *args, **kwargs):
        """Permite a atualização parcial de uma clínica (PATCH)."""
...
def enviar_mensagem_evolution(clinica, numero_destino, texto):
    """Envia mensagem via Evolution API rodando localmente"""
    # Se não houver URL configurada, usa o padrão do Docker interno
    base_url = clinica.evolution_server_url or "http://host.docker.internal:8080"
    instance = clinica.evolution_instance_name
    
    if not instance:
        print("❌ Nome da instância não configurado.")
        return False

    url = f"{base_url}/message/sendText/{instance}"
    
    headers = {
        "apikey": clinica.evolution_apikey or "intellimed-secret-key-global",
        "Content-Type": "application/json"
    }
    
    # Formatação do número para o padrão do WhatsApp (55 + DDD + numero)
    # Remove caracteres não numéricos
    num_limpo = re.sub(r'\D', '', numero_destino)
    if not num_limpo.endswith('@s.whatsapp.net'):
        num_limpo = f"{num_limpo}@s.whatsapp.net"
    ...
    try:
        print(f"📤 Enviando via Evolution para {num_limpo}...")
        response = requests.post(url, json=payload, headers=headers, timeout=10)
        print(f"✅ Status Envio: {response.status_code}")
        return response.status_code == 201
    except Exception as e:
        print(f"❌ Erro Evolution: {e}")
        return False

def gerar_resposta_ia(clinica, mensagem_usuario, numero_paciente):
    ...
    try:
        ...
        nome_secretaria = clinica.whatsapp_secretaria_nome or "Assistente"
        
        prompt = f"""
        Você é {nome_secretaria}, secretária da clínica "{clinica.nome}".
        Responda de forma curta, educada e profissional no WhatsApp.
...
def whatsapp_webhook(request):
    ...
    try:
        ...
        # Ignorar mensagens enviadas por MIM (pelo sistema)
        if key.get('fromMe'):
        ...
        if not texto:
            return HttpResponse("No text", status=200)
            
        print(f"\n📩 MENSAGEM DE {numero_paciente}: {texto}")
        
        # Identificar clínica pela instância
        instance = payload.get('instance')
        try:
            clinica = Clinica.objects.get(evolution_instance_name=instance)
        except Clinica.DoesNotExist:
            print(f"❌ Instância {instance} não vinculada a nenhuma clínica.")
            return HttpResponse("Instance not found", status=200)
            
        if not clinica.plano_whatsapp_ia_ativo:
            return HttpResponse("Inactive plan", status=200)
            
        # IA e Resposta
        resposta = gerar_resposta_ia(clinica, texto, numero_paciente)
        enviar_mensagem_evolution(clinica, numero_paciente, resposta)
