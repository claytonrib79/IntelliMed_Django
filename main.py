...
def transcrever_consulta_com_gemini(consulta_id):
    """
    Transcreve consulta identificando falas do médico e paciente (VERSÃO SÍNCRONA)
    """
    import time
    
    # Busca a consulta no início para ter o objeto
    try:
        consulta_obj = Consulta.objects.get(id=consulta_id)
        consulta_obj.status = 'transcrevendo'
        consulta_obj.save(update_fields=['status'])
    except Consulta.DoesNotExist:
        return {'sucesso': False, 'erro': 'Consulta não encontrada no início do processo.'}

    try:
        inicio = time.time()
        
        if not Config.GEMINI_API_KEY: raise Exception("GEMINI_API_KEY não configurada")
        if not genai: raise Exception("google-generativeai não instalado")
        
        genai.configure(api_key=Config.GEMINI_API_KEY)
        model = genai.GenerativeModel('gemini-2.5-pro')
        
        paciente = consulta_obj.paciente
        if not paciente: raise Exception("Consulta sem paciente associado.")

        prompt = f"""
        Você é um sistema de IA para apoio médico, especializado em transcrever e analisar consultas.
        CONTEXTO DA CONSULTA:
        - Médico Responsável: Dr. {consulta_obj.medico_responsavel or '[Nome do Médico]'}
        - Paciente: {paciente.nome_completo}
        INSTRUÇÕES DE IDENTIFICAÇÃO (MUITO IMPORTANTE):
        - O MÉDICO é quem geralmente faz perguntas e usa termos técnicos.
        - O PACIENTE é quem geralmente descreve sintomas.
        - **NÃO USE RÓTulos GENÉRICOS como 'VOZ_MASCULINA'.** Use OBRIGATORIAMENTE os rótulos 'medico', 'paciente', ou 'acompanhante'.
        FORMATO DA RESPOSTA (JSON VÁLIDO OBRIGATORIAMENTE):
        {{
            "dialogo": [ {{"tipo": "medico", "texto": "..."}}, {{"tipo": "paciente", "texto": "..."}} ],
            "resumo_estruturado": {{ "queixa_principal": "...", "historia_doenca_atual": "...", "exame_fisico": "...", "hipotese_diagnostica": "...", "conduta": "...", "medicacoes": ["..."] }},
            "falas_medico": "...",
            "falas_paciente": "...",
            "confianca": 0.95
        }}
        PROCEDA COM A TRANSCRIÇÃO E ANÁLISE DO ÁUDIO:
        """
        
        if consulta_obj.audio_consulta:
            audio_bytes = base64.b64decode(consulta_obj.audio_consulta.split(',', 1)[-1])
            request_options = {"timeout": 300}

            response = model.generate_content(
                [prompt, {"mime_type": f"audio/{consulta_obj.audio_formato or 'webm'}", "data": audio_bytes}],
                request_options=request_options
            )

            if not response.text or not response.text.strip():
                raise Exception("A IA retornou uma resposta vazia. O áudio pode estar muito baixo, corrompido ou houve uma falha na API.")
            ...
            try:
                resultado = json.loads(response_text)
                 ...
                 raise Exception("A IA retornou um formato inválido (não-JSON).")

            # ▼▼▼ INÍCIO DA CORREÇÃO DEFINITIVA ▼▼▼
            
            # 1. Monta um dicionário com todos os campos que precisam ser atualizados.
            resumo = resultado.get('resumo_estruturado', {})
            dialogo = resultado.get('dialogo', [])
            transcricao_formatada = "\n".join([f"[{fala.get('tipo', 'desconhecido').upper()}]: {fala.get('texto', '')}" for fala in dialogo])

            update_data = {
                'transcricao_completa': resultado.get('transcricao_completa', ''),
                'transcricao_medico': resultado.get('falas_medico', ''),
                'transcricao_paciente': resultado.get('falas_paciente', ''),
                'confianca_transcricao': float(resultado.get('confianca', 0)),
                'queixa_principal': resumo.get('queixa_principal', consulta_obj.queixa_principal),
                'historia_doenca_atual': resumo.get('historia_doenca_atual', consulta_obj.historia_doenca_atual),
                'exame_fisico': resumo.get('exame_fisico', consulta_obj.exame_fisico),
                'hipotese_diagnostica': resumo.get('hipotese_diagnostica', consulta_obj.hipotese_diagnostica),
                'conduta': resumo.get('conduta', consulta_obj.conduta),
                'transcricao_ia': transcricao_formatada,
                'tempo_processamento_transcricao': time.time() - inicio,
                'status': 'aguardando_revisao'
            }
            
            # 2. Usa o método .update() para salvar os dados diretamente no banco de dados.
            #    Isso é mais seguro e bypassa o método .save() que está causando o problema.
            Consulta.objects.filter(id=consulta_id).update(**update_data)
            
            # ▲▲▲ FIM DA CORREÇÃO DEFINITIVA ▲▲▲
            
            return {'sucesso': True}
        else:
            raise Exception("Nenhum áudio disponível na consulta para processar.")
    
    except Exception as e:
        try:
            # Reverte o status para permitir uma nova tentativa
            Consulta.objects.filter(id=consulta_id).update(status='em_atendimento')
        except: pass
...
class ReceitaViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        # OTIMIZAÇÃO: .select_related() busca os dados da categoria e do paciente
        # na mesma consulta ao banco de dados, evitando N+1 queries.
...
class DespesaViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        # OTIMIZAÇÃO: .select_related() busca os dados da categoria
...
def dashboard_financeiro(request):
    ...
    return Response({
        ...
        'previsao_mensal': previsao_mensal, # NOVO CAMPO ADICIONADO
        'a_receber': float(receitas_a_receber)
    })
...
def dados_clinica_completo(request):
    ...
    def calcular_estatisticas_detalhadas(start_date, end_date):
        ...
        # Cria um dicionário com os totais existentes
        stats_dict = {item['status']: item['total'] for item in ag_stats}
        
        # Garante que todos os status possíveis estejam na resposta
    ...
    return Response({
        ...
        'estatisticas_gerais': {
            ...
            'estatisticas_detalhadas': estatisticas_detalhadas, # <-- CAMPO AGORA TEM A LISTA COMPLETA
        },
...
# ▼▼▼ SUBSTITUA A FUNÇÃO 'auditoria_recente_view' INTEIRA PELA VERSÃO CORRIGIDA E FINAL ▼▼▼
@api_view(['GET'])
@authentication_classes([JWTAuthentication])
@permission_classes([IsAuthenticated])
