...
# Substitua a classe inteira por esta:
class Consulta(TenantModel):
    ...
    def iniciar_atendimento(self):
        ...
        # Adicionamos os campos do médico ao `update_fields` para garantir que sejam salvos.
...
def transcrever_consulta_com_gemini(consulta_id):
    ...
    try:
        ...
        - Médico Responsável: Dr. {consulta.medico_responsavel or '[Nome do Médico]'}
        ...
        if consulta.audio_consulta:
            audio_bytes = base64.b64decode(consulta.audio_consulta.split(',', 1)[-1])
            request_options = {"timeout": 300}

            response = model.generate_content(
                [prompt, {"mime_type": f"audio/{consulta.audio_formato or 'webm'}", "data": audio_bytes}],
                request_options=request_options
            )

            if not response.text or not response.text.strip():
                raise Exception("A IA retornou uma resposta vazia. O áudio pode estar muito baixo ou corrompido.")
            ...
            consulta.queixa_principal = resumo.get('queixa_principal', consulta.queixa_principal)
            consulta.historia_doenca_atual = resumo.get('historia_doenca_atual', consulta.historia_doenca_atual)
            consulta.exame_fisico = resumo.get('exame_fisico', consulta.exame_fisico)
            consulta.hipotese_diagnostica = resumo.get('hipotese_diagnostica', consulta.hipotese_diagnostica)
            consulta.conduta = resumo.get('conduta', consulta.conduta)
            ...
            # Especifica exatamente quais campos devem ser atualizados,
            # preservando os dados do médico e outros campos.
            campos_para_salvar = [
                'transcricao_completa', 'transcricao_medico', 'transcricao_paciente',
                'confianca_transcricao', 'queixa_principal', 'historia_doenca_atual',
                'exame_fisico', 'hipotese_diagnostica', 'conduta', 'transcricao_ia',
                'tempo_processamento_transcricao', 'status'
            ]
            consulta.save(update_fields=campos_para_salvar)
            # ▲▲▲ FIM DA CORREÇÃO ▲▲▲
...
class ExameSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        read_only_fields = [
            'id', 
            'clinica_id', # Adicionar clinica_id a read_only
            'interpretacao_ia',
...
class ConsultaViewSet(viewsets.ModelViewSet):
    ...
    def listar_documentos(self, request, pk=None):
        ...
        # Mapeamento de campos para tipos
        campo_map = {
...
class ExameViewSet(viewsets.ModelViewSet):
    ...
    # A permissão padrão agora é para leitura (Secretaria ou superior)
    permission_classes = [IsAuthenticated, IsSecretariaOrAbove] 
    authentication_classes = [JWTAuthentication]
    filter_backends = [filters.SearchFilter, filters.OrderingFilter]
    ...
    def get_permissions(self):
        ...
        Secretarias podem apenas LER (list, retrieve).
        Médicos/Admins podem fazer TUDO.
        """
        if self.action in ['list', 'retrieve']:
            permission_classes = [IsAuthenticated, IsSecretariaOrAbove]
        else:
            # Para criar, editar, deletar, etc., exige permissão de Médico/Admin
            permission_classes = [IsAuthenticated, IsAdminOrMedico]
