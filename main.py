...
class ExameViewSet(viewsets.ModelViewSet):
    ...
    def upload_interpretar_ia(self, request):
        ...
        # Usa async_to_sync para chamar a função assíncrona de forma segura
        try:
            resultado = async_to_sync(interpretar_exame_com_gemini)(exame.id)
            ...
            # Captura qualquer exceção que a função async possa gerar
            exame.delete()
            return Response({
                'erro': 'Erro crítico durante a interpretação do exame',
                'detalhes': str(e)
            }, status=500)
