...
if not settings.configured:
    settings.configure(
        ...
        INSTALLED_APPS=[
            ...
            'rest_framework',
            'channels',
            'django_apscheduler', # <<< ADICIONADO AQUI
        ],
        ...
        # Configurações de E-mail (substitua com seu provedor real em produção)
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(
                fields=['clinica_id', 'data', 'hora'],
                condition=~Q(status__in=['Cancelado', 'Realizado']),
                name='unique_active_agendamento'
            )
        ]
    # ▲▲▲ FIM DA CORREÇÃO ▲▲▲
...
class Usuario(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        # Só validar se NÃO for update de campos específicos
...
class UsuarioSerializer(serializers.ModelSerializer):
    ...
    def update(self, instance, validated_data):
        ...
        # Salvar SEM chamar full_clean (para evitar validação de CPF no update)
        instance.save(update_fields=list(validated_data.keys()) + (['password'] if password else []))
        return instance
...
class AgendamentoSerializer(serializers.ModelSerializer):
    ...
    # ▼▼▼ CORREÇÃO APLICADA AQUI ▼▼▼
    def validate(self, data):
        """Validações gerais, incluindo verificação de disponibilidade."""
        request = self.context.get('request')
        
        # Garante que temos a data e a hora para validar.
        # Se for uma atualização parcial (PATCH), pode ser que não venham.
        ...
        if request and hasattr(request, 'user') and data_agendamento and hora_agendamento:
            ...
            # ID do agendamento atual, se estivermos editando
            excluir_id = self.instance.id if self.instance else None
            
            if not Agendamento.verificar_disponibilidade(
                clinica_id, data_agendamento, hora_agendamento, excluir_id
            ):
                # Levanta um erro de validação claro que será enviado como resposta 400
                raise serializers.ValidationError({
                    'hora': f"Horário indisponível. Já existe um agendamento para {data_agendamento.strftime('%d/%m/%Y')} às {hora_agendamento.strftime('%H:%M')}."
                })
...
class AgendamentoViewSet(viewsets.ModelViewSet):
    ...
    # ▼▼▼ CORREÇÃO APLICADA AQUI (MÉTODO NOVO) ▼▼▼
    def perform_create(self, serializer):
...
def backup_criar_view(request):
    """
    Cria e retorna um backup JSON dos dados da clínica.
    GET /api/backup/criar/
    """
    user = request.user
    clinica_id = user.get('clinica_id')
    if not clinica_id:
    ...
    # Coleta de dados com serializers para garantir consistência
    ...
    # Prepara a resposta para forçar o download
...
# NO ARQUIVO: main.py
# ANTES DA FUNÇÃO: backup_configurar_view
# DEPOIS DA FUNÇÃO: backup_criar_view
...
def backup_restaurar_view(request):
    """Restaura dados de um backup JSON. ATENÇÃO: Operação destrutiva."""
    user = request.user
    clinica_id = user.get('clinica_id')
    
    if 'backup_file' not in request.FILES:
        return Response({'erro': 'Nenhum arquivo de backup enviado.'}, status=400)
    ...
    try:
        ...
        return Response({'erro': 'Arquivo de backup inválido ou corrompido.'}, status=400)

    backup_clinica_id = backup_data.get('clinica_id')
    if backup_clinica_id != clinica_id:
    ...
    id_map = {'pacientes': {}, 'categorias_receita': {}, 'categorias_despesa': {}, 'consultas': {}, 'agendamentos': {}}
    dados = backup_data.get('dados', {})

    # ▼▼▼ CORREÇÃO APLICADA AQUI ▼▼▼
    # Função auxiliar para extrair apenas campos válidos de um modelo
    ...
    for cat in dados.get('categorias_receita', []):
        old_id = cat.pop('id'); cat.pop('clinica_id', None)
    ...
    for cat in dados.get('categorias_despesa', []):
        old_id = cat.pop('id'); cat.pop('clinica_id', None)
    ...
    for paciente_data in dados.get('pacientes', []):
        old_id = paciente_data.pop('id'); paciente_data.pop('clinica_id', None)
    ...
    for agendamento_data in dados.get('agendamentos', []):
        old_id = agendamento_data.pop('id'); agendamento_data.pop('clinica_id', None)
    ...
    for consulta_data in dados.get('consultas', []):
        old_id = consulta_data.pop('id'); consulta_data.pop('clinica_id', None)
    ...
    for exame_data in dados.get('exames', []):
        exame_data.pop('id', None); exame_data.pop('clinica_id', None)
    ...
    for receita_data in dados.get('receitas', []):
        receita_data.pop('id', None); receita_data.pop('clinica_id', None)
    ...
    for despesa_data in dados.get('despesas', []):
        despesa_data.pop('id', None); despesa_data.pop('clinica_id', None)
    ...
    return Response({'mensagem': 'Restauração concluída com sucesso!'}, status=200)
...
def _enviar_backup_por_email(clinica):
    """Gera o backup e envia por e-mail."""
    if not clinica.backup_email_notificacao:
        print(f"    ! AVISO: Clínica {clinica.id} não tem e-mail de notificação configurado. Backup abortado.")
        return
    ...
    try:
        ...
        filename = f"backup_intellimed_{clinica.id}_{timezone.now().strftime('%Y%m%d')}.json"
        ...
        print(f"    ✗ ERRO ao enviar e-mail de backup para clínica {clinica.id}: {e}")
...
def backup_restaurar_view(request):
    ...
    for paciente in dados.get('pacientes', []):
        old_id = paciente.pop('id'); paciente.pop('clinica_id', None)
        novo_paciente = Paciente.objects.create(clinica_id=clinica_id, **paciente)
        id_map['pacientes'][old_id] = novo_paciente.id
        
    for agendamento in dados.get('agendamentos', []):
        old_id = agendamento.pop('id'); agendamento.pop('clinica_id', None)
        agendamento['paciente_id'] = id_map['pacientes'].get(agendamento.pop('paciente'))
        if agendamento['paciente_id']:
            novo_agendamento = Agendamento.objects.create(clinica_id=clinica_id, **agendamento)
            id_map['agendamentos'][old_id] = novo_agendamento.id
    
    for consulta in dados.get('consultas', []):
        old_id = consulta.pop('id'); consulta.pop('clinica_id', None)
        consulta['paciente_id'] = id_map['pacientes'].get(consulta.pop('paciente'))
        if consulta.get('agendamento'):
            consulta['agendamento_id'] = id_map['agendamentos'].get(consulta.pop('agendamento'))
        if consulta['paciente_id']:
            nova_consulta = Consulta.objects.create(clinica_id=clinica_id, **consulta)
            id_map['consultas'][old_id] = nova_consulta.id

    for exame in dados.get('exames', []):
        exame.pop('id', None); exame.pop('clinica_id', None)
        exame['paciente_id'] = id_map['pacientes'].get(exame.pop('paciente'))
        if exame.get('consulta'):
            exame['consulta_id'] = id_map['consultas'].get(exame.pop('consulta'))
        if exame['paciente_id']:
            Exame.objects.create(clinica_id=clinica_id, **exame)

    for receita in dados.get('receitas', []):
        receita.pop('id', None); receita.pop('clinica_id', None)
        receita['paciente_id'] = id_map['pacientes'].get(receita.pop('paciente'))
        if receita.get('categoria'):
            receita['categoria_id'] = id_map['categorias_receita'].get(receita.pop('categoria'))
        if receita.get('agendamento'):
            receita['agendamento_id'] = id_map['agendamentos'].get(receita.pop('agendamento'))
        Receita.objects.create(clinica_id=clinica_id, **receita)
        
    for despesa in dados.get('despesas', []):
        despesa.pop('id', None); despesa.pop('clinica_id', None)
        if despesa.get('categoria'):
            despesa['categoria_id'] = id_map['categorias_despesa'].get(despesa.pop('categoria'))
...
if __name__ == '__main__':
    ...
    # ▼▼▼ LÓGICA DE INICIALIZAÇÃO DO AGENDADOR ADICIONADA AQUI ▼▼▼
    try:
        print("\n" + "="*70)
        ...
        # Remove a tarefa antiga se já existir, para evitar duplicatas
        if scheduler.get_job('backup_automatico_diario'):
            scheduler.remove_job('backup_automatico_diario')
            print("   - Tarefa de backup antiga removida.")

        # Adiciona a tarefa para rodar a cada 24 horas
        scheduler.add_job(
        ...
        scheduler.start()
        print("   ✅ Agendador iniciado. Verificação de backups a cada 24 horas.")
        print("="*70)
    except Exception as e:
