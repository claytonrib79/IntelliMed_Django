...
def exames_analise_view(request):
    ...
    if request.method == 'POST':
        ...
        if not paciente_id or not arquivo:
            ...
            try:
                files = {'arquivo': (arquivo.name, arquivo.read(), arquivo.content_type)}
                ...
                messages.success(request, "Exame analisado com sucesso!")
                return redirect('exame_visualizar', exame_id=resultado['exame']['id'])
                
            except requests.exceptions.HTTPError as e:
                try:
                    error_data = e.response.json()
                    messages.error(request, f"Erro na análise: {error_data.get('erro', 'Verifique o arquivo e tente novamente.')}")
                except json.JSONDecodeError:
                    messages.error(request, f"Erro no servidor (código {e.response.status_code}). Tente novamente.")
            except Exception as e:
    ...
    try:
        pac_res = requests.get(f'{Config.BACKEND_URL}/api/pacientes/?page_size=1000', headers={'Authorization': f'Bearer {token}'})
        if pac_res.status_code == 200:
            context['pacientes'] = pac_res.json().get('results', pac_res.json())

        exames_res = requests.get(f'{Config.BACKEND_URL}/api/exames/', headers={'Authorization': f'Bearer {token}'})
        if exames_res.status_code == 200:
            context['exames'] = exames_res.json().get('results', exames_res.json())
    except Exception as e:
        messages.error(request, f"Erro ao carregar dados da página: {e}")
