...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        /* AQUI ESTÁ A ALTERAÇÃO ESTÉTICA FINAL */
        .actions-btn { 
            background: transparent; /* Remove o fundo */
            border: none;
            color: #667eea; /* Cor azul do menu para a engrenagem */
            border-radius: 8px; /* Mantém uma área de clique suave */
            width: 40px; 
            height: 40px; 
            cursor: pointer; 
            transition: color 0.2s, background-color 0.2s; /* Adiciona transição suave */
            display: flex;
            align-items: center;
            justify-content: center;
        }
        .actions-btn:hover {
            background-color: rgba(102, 126, 234, 0.1); /* Um fundo azul bem claro no hover */
            color: #5a67d8; /* Azul um pouco mais escuro no hover */
        }
...
<body>
        ...
        <div class="form-container">
            <form method="POST">
                ...
                <div class="form-grid">
                    ...
                    <!-- CAMPO CONDICIONAL TIPO DE CONSULTA -->
                    ...
                    <!-- CAMPO CONDICIONAL TIPO DE EXAME -->
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Adiciona o evento e chama a função na carga da página
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        /* <<< ALTERAÇÃO NO GRID PARA ACOMODAR OS NOVOS CARDS >>> */
...
<body>
    ...
    <div class="main-content">
        ...
        <div class="stats-grid">
            <!-- <<< INÍCIO DA ALTERAÇÃO HTML DOS CARDS >>> -->
            ...
            <!-- <<< FIM DA ALTERAÇÃO HTML DOS CARDS >>> -->
        </div>
        ...
        <div class="terms-container">
            ...
            <div class="terms-footer">
                <form action="{% url 'exames_analise' %}" method="get">
                    <button type="submit" class="btn btn-primary">Entendi e Concordo, Continuar</button>
                </form>
            </div>
        </div>
    </div>
...
<body>
    <!-- <<< INÍCIO DA CORREÇÃO: Menu lateral adicionado >>> -->
    <div class="sidebar">
        <div class="sidebar-header"><h1>intelliMed</h1></div>
        <div class="user-info"><strong>{{ user.nome_completo }}</strong><small>{{ user.funcao_display }}</small></div>
        <nav class="menu">
            ...
            <table>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <a href="/exames/{{ exame.id }}/visualizar/">Ver Laudo</a>
                        </td>
                    </tr>
                    {% empty %}
    ...
    <script>
        ...
        form.addEventListener('submit', (e) => {
            // Adicionado para evitar o comportamento padrão do formulário
            // e garantir que a URL correta seja usada
...
def exames_analise_view(request):
    ...
    if request.method == 'POST':
        ...
        if not paciente_id or not arquivo:
            ...
            try:
                # <<< INÍCIO DA CORREÇÃO >>>
                # O tipo de exame enviado será 'outros', pois a IA fará a identificação.
                # O campo `tipo_exame` no backend é obrigatório, e 'outros' é uma opção válida.
                tipo_exame = 'outros'
                # <<< FIM DA CORREÇÃO >>>
                
                print(f"\n🔬 DEBUG EXAMES IA - UPLOAD:")
                ...
                data = {
                    ...
                    'tipo_exame': tipo_exame, # <<< AGORA ENVIA 'outros'
                    'observacoes': 'Exame enviado para análise por IA.' # <<< Adicionado um texto padrão
                }
                ...
                print(f"📤 Enviando para: {Config.BACKEND_URL}/api/exames/upload-ia/")
                ...
                if response.status_code in [200, 201]:
                    ...
                    # <<< CORREÇÃO NA LÓGICA DE REDIRECIONAMENTO >>>
                    # A resposta correta agora está dentro da chave 'exame'
                    ...
                    error_data = response.json()
                    error_msg = error_data.get('detail', error_data.get('erro', 'Dados inválidos'))
                    messages.error(request, f"❌ Erro na validação: {error_msg}")
                    ...
                    messages.error(request, "📦 Arquivo muito grande! O limite é 10MB.")
                    
                elif response.status_code == 500:
                    messages.error(request, "⚠️ Erro no servidor ao processar o exame com IA.")
                    try:
                        error_json = response.json()
                        if 'detalhes' in error_json: # A chave de erro na API é 'detalhes'
                            messages.error(request, f"Detalhes: {error_json['detalhes']}")
                        else:
                            messages.error(request, "Verifique se a API do Google Gemini está configurada corretamente.")
                    except:
                        messages.error(request, "O servidor retornou um erro interno e não foi possível obter detalhes.")
                        
                else:
                    messages.error(request, f"❌ Erro desconhecido: {response.status_code}")
    ...
    try:
        ...
        if pac_res.status_code == 200:
            ...
            print(f"📋 Total de pacientes carregados: {len(context['pacientes'])}")
    ...
    try:
        ...
        if exames_res.status_code == 200:
            ...
            print(f"📊 Total de exames encontrados: {len(context['exames'])}")
            
    except Exception as e:
        print(f"⚠️ Erro ao carregar histórico de exames: {e}")
        # Não mostrar erro para o usuário, apenas deixar a lista vazia
...
def exame_visualizar_view(request, exame_id):
    return placeholder_view(request, exame_id, page_title=f"Laudo do Exame #{exame_id}")

# Placeholders para as futuras páginas de visualização, edição e deleção
def exame_visualizar_view(request, exame_id):
    # AQUI Futuramente buscaremos os dados do laudo e mostraremos em uma página dedicada
    return placeholder_view(request, exame_id, page_title=f"Laudo do Exame #{exame_id}")
