                ...
                pacientesAtrasados.forEach(paciente => {
                    ...
                    const itemHtml = `
                        <div class="list-item">
                            ...
                            <span class="status-badge-pill pill-atraso">${formattedTime} horas atrasado</span>
                        </div>
                    `;
...
<body>
    ...
    <div class="main-content">
        <div class="page-header">
            ...
            {% if user.funcao == 'admin' or user.funcao == 'medico' %}
        ...
        {% else %}
            <div class="info-container">
                ...
                <!-- --- CARD DE FATURAMENTO RESTAURADO AQUI --- -->
                <div class="info-section">
...
<body>
    ...
    <!-- ▼▼▼ INÍCIO DA ALTERAÇÃO (BLOCO SCRIPT) ▼▼▼ -->
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            // Apenas inicia o timer se não for uma página de erro de permissão
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                # ▼▼▼ LÓGICA COMPLETA DE REDIRECIONAMENTO APLICADA AQUI ▼▼▼
                if user_info.get('precisa_alterar_senha'):
                    # Se o usuário precisa alterar a senha, não completamos o login.
                    # Em vez disso, salvamos o e-mail em uma chave de sessão temporária
                    # e o redirecionamos para a página de definição de senha.
                    ...
                    # Se não precisa alterar, procede com o login normal.
    ...
    try:
        ...
        if agendamentos:
            ...
            for ag in agendamentos:
                ...
                # --- LÓGICA COMPLETA PARA EXIBIR O MÉDICO ---
...
def agendamento_form_view(request, agendamento_id=None):
    ...
    pacientes = []
    medicos = [] # --- NOVO: Inicializa a lista de médicos ---
    try:
        # Busca pacientes
        ...
        # --- NOVO: Busca a lista de médicos da nova API ---
    ...
    if request.method == 'POST':
        ...
        payload = {
            ...
            'medico_responsavel': request.POST.get('medico_responsavel') or None, # --- NOVO: Adiciona o médico ao payload ---
            'servico': servico,
            'tipo': tipo,
            'convenio': request.POST.get('convenio'),
        ...
        try:
            ...
            form_data_on_error = {
                ...
                'medico_responsavel': int(request.POST.get('medico_responsavel')) if request.POST.get('medico_responsavel') else None, # --- NOVO: Preserva o médico em caso de erro ---
                'servico': request.POST.get('servico'),
                'tipo': tipo,
                'convenio': request.POST.get('convenio'),
                'data': request.POST.get('data'),
            ...
            context = RequestContext(request, {
                ...
                'pacientes': pacientes,
                'medicos': medicos, # --- NOVO: Envia médicos para o template em caso de erro ---
    ...
    context = RequestContext(request, {
        'form_title': form_title,
        'pacientes': pacientes,
        'medicos': medicos, # --- NOVO: Envia a lista de médicos para o template ---
        'agendamento': agendamento_data,
        'user': user,
        'messages': messages.get_messages(request)
    })
...
def faturamento_view(request):
    ...
    if data_inicial_str and data_inicial_str == data_final_str:
        selected_date = data_inicial_str
        params['data_inicial'] = data_inicial_str
        params['data_final'] = data_final_str
    elif data_inicial_str and data_final_str:
        # Lógica para identificar os botões de período
        try:
        ...
        params['mes'] = mes_str
        params['ano'] = ano_str
        filtro_ativo = 'mes' # Marca o botão "Mensal" se um mês/ano for escolhido
...
def informacoes_view(request):
    ...
    context = {
        'user': user,
        'data': {}, 'total_inativos': 0, 'error_message': None,
    ...
    # --- A LINHA ABAIXO FOI A QUE EU ESQUECI E CAUSOU O ERRO. AGORA ESTÁ CORRETA. ---
    ctx = RequestContext(request, context)
    return HttpResponse(template.render(ctx))
...
def backup_view(request):
    ...
    if user.get('funcao') not in ['admin', 'medico']:
